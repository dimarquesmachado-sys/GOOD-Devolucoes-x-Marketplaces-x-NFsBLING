...
module.exports = function registrarCicloDefeitos(router, deps) {
  ...
  // ─────────────────────────────────────────────────────────────────────
  // b168 - POST /amb/api/defeitos/:id/restaurar  (so ADMIN)
  ...
  // ─────────────────────────────────────────────────────────────────────
  router.post('/api/defeitos/:id/restaurar', auth.requerLogin, async (req, res) => {
    const s = auth.validarSessao(auth.tokenDaRequisicao(req), 'admin');
    if (!s) return res.status(403).json({ ok: false, erro: 'so o admin pode restaurar' });
    const dbc = cli();
    if (!dbc) return erroSemBanco(res);
    try {
      const rI = await dbc.from(db.tabelas.devolucoes).select('id, tipo, tipo_anterior, status').eq('id', req.params.id).limit(1).then(r => r).catch(() => ({ data: null, error: { message: 'erro' } }));
      let item = (rI.data || [])[0] || null;
      if (!item && rI.error) {
        const r2 = await dbc.from(db.tabelas.devolucoes).select('id, tipo, status').eq('id', req.params.id).limit(1);
        item = (r2.data || [])[0] || null;
      }
      ...
      let rU = await dbc.from(db.tabelas.devolucoes).update({ ...camposR, tipo_anterior: null }).eq('id', req.params.id);
      if (rU.error) rU = await dbc.from(db.tabelas.devolucoes).update(camposR).eq('id', req.params.id);
  ...
  router.post('/api/defeitos/:id/excluir', auth.requerLogin, async (req, res) => {
    ...
    try {
      ...
      // b168 - guarda o tipo ANTERIOR pro botao RESTAURAR poder voltar
      ...
      let rU = await dbc.from(db.tabelas.devolucoes).update(camposExc).eq('id', req.params.id);
      if (rU.error) {
        delete camposExc.tipo_anterior;
        rU = await dbc.from(db.tabelas.devolucoes).update(camposExc).eq('id', req.params.id);
      }
      if (rU.error) throw new Error(rU.error.message);
      try {
        await dbc.from(T_COM).insert([{
  ...
  router.delete('/api/defeitos/comentario/:cid', auth.requerLogin, async (req, res) => {
    ...
    try {
      ...
      // b206 (bug que o Diego viu: "apagou uma anotacao" aninhado
      ...
      if (alvo) {
        await dbc.from(T_COM).insert([{
          ...
          // b206 - texto do rastro sem aspas aninhadas
