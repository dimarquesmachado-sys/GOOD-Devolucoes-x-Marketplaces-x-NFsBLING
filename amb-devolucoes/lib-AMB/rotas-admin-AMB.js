...
app.post('/api/admin/full-lancar-estoque/:id', requerAdmin, async (req, res) => {
  ...
  try {
    ...
    if (listaDeps.length) {
      ...
      // b281 (review do Codex) - se o admin ESCOLHEU um deposito e ele nao
      // serve (nao existe nesta empresa, ou e um Full), cair no Geral em
      // silencio lanca a peca num lugar que ninguem pediu — e o card fecha
      // como se tivesse dado certo. Recusa com o motivo escrito; o Geral so
      // vale quando NADA foi escolhido.
      if (pedidoDep && !valido) {
