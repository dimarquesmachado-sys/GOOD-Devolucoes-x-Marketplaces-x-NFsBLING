...
async function buscarNFnoBlingPorOrderId(orderIdML, dataReferencia, opcoes = {}) {
  ...
  for (let pagina = 1; pagina <= MAX_PAGINAS; pagina++) {
    ...
    const filtroData = (dataLimite && dataReferencia)
      ? '&dataEmissaoInicial=' + dataLimite.toISOString().slice(0, 10)
        + '&dataEmissaoFinal=' + new Date(new Date(dataReferencia).getTime()
          + 60 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10)
      : '';
