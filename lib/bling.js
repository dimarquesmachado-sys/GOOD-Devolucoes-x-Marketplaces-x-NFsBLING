...
async function buscarNFnoBlingPorOrderId(orderIdML, dataReferencia, opcoes = {}) {
  ...
  for (let pagina = 1; pagina <= MAX_PAGINAS; pagina++) {
    ...
    const fimJanela = usaFatias ? refT + 30 * 864e5 : Date.now();
    ...
    // anterior. Sem isto, a busca parava na primeira e nao alcancaria uma
    // venda de meses atras — o caso que motivou tudo isto.
    if (lista.length < LIMITE_PAGINA || paginasNaFatia >= PAGINAS_POR_FATIA) {
      if (!usaFatias) break;
      fatiaAtual++;
      paginasNaFatia = 0;
