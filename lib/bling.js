    ...
    if (error.response?.status === 401) {
      ...
      if (tokenLeitor.politicaDe('good', 'bling') === 'remoto') {
        try {
          ...
          // comentario no meio.
          tokenLeitor.anotarRetry('good', 'bling', true);
          return { ok: true, data: r.data, status: r.status };
        } catch (err2) {
            tokenLeitor.anotarRetry('good', 'bling', false);
...
async function buscarProdutoBlingPorSku(sku) {
  ...
  if (match.id) {
    ...
    if (rDetalhe.ok && rDetalhe.data?.data) {
      // `detalhado: true` avisa quem chama que `produto` JA E a resposta
      // completa de /produtos/{id} (com formato/estrutura) - review do
      // Codex no #250 apontou uma rota que pedia esse MESMO detalhe de
      // novo por nao saber disso.
      return { ok: true, produto: rDetalhe.data.data, detalhado: true };
    }
  }

  return { ok: true, produto: match, detalhado: false };
}
