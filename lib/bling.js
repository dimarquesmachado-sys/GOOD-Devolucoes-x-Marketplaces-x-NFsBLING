...
async function buscarNFnoBlingPorOrderId(orderIdML, dataReferencia, opcoes = {}) {
  ...
  for (let pagina = 1; pagina <= MAX_PAGINAS; pagina++) {
    ...
    const filtroData = Number.isFinite(refT)
      ? '&dataEmissaoInicial=' + new Date(refT - DIAS_ANTES * 864e5).toISOString().slice(0, 10)
        // b197.2 (Codex): a NF pode sair DEPOIS da data usada como
        // referencia — venda faturada dias depois, ou a data ser a da
        // devolucao. 2 dias era apertado; 30 cobre sem perder o ganho de
        // olhar pra tras.
        + '&dataEmissaoFinal=' + new Date(refT + 30 * 864e5).toISOString().slice(0, 10)
      : '';
