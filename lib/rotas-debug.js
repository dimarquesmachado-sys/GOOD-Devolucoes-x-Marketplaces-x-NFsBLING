...
module.exports = function registrarRotasDebug(app, deps) {
  ...
  app.get('/api/debug/achar-nf', (req, res, next) => {
    ...
    try {
      ...
      const r = await buscarNFnoBlingPorOrderId(pedido, data, {
        maxPaginas: parseInt(req.query.paginas, 10) || 20,
      });
      ...
      return res.json({
        ...
        leia: 'Se `notas_vistas` for alto e as datas NAO cobrirem a venda, a janela esta errada. '
          + 'Se cobrirem e mesmo assim nao achou, o `numeroPedidoLoja` no Bling esta em outro '
          + 'formato — compare com o pedido que voce passou.',
      });
    } catch (e) {
