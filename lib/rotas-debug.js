...
// ── b220.2 (Codex): as regras do teste da chave, no ESCOPO DO MODULO ──
//
// Elas vivem aqui, e nao dentro de `registrarRotasDebug`, pra o teste
// exercitar as DE PRODUCAO. Meu teste anterior reimplementava a logica —
// passaria verde mesmo se a rota quebrasse, que e o oposto do que serve.
// b220.2 (Codex): estas duas sao EXPORTADAS no fim do modulo, pra o teste
// exercitar as de producao em vez de reimplementar. Meu teste anterior
// copiava a logica — passaria verde mesmo se a rota quebrasse, que e o
// oposto do que um teste serve.

// 4xx e recusa definitiva (o parametro nao existe), MENOS 429 (fila) e
// 408 (timeout), que pedem nova tentativa.
function sondaFalhouPorAcaso(x) {
...
/**
 * O veredito de UMA sonda. Em funcao nomeada de proposito: aqui moram as
 * 6 correcoes que o Codex apontou, e cada uma se perde num ternario.
 */
function vereditoDaSonda({ ehControle, httpOk, lista, bateu, detalheFalhou, totalDaConta }) {
  if (!httpOk) return 'erro';

  // o CONTROLE busca por numero, que se repete entre series — varias
  // linhas ali e o esperado, nao sinal de filtro ignorado
  if (ehControle) {
  ...
  // chave e UNICA: mais de uma linha ja prova que o filtro foi ignorado,
  // independente de quais notas vieram
  if (lista > 1) {
    return 'ignorou o filtro (vieram ' + lista + ' — chave e unica, '
      + 'filtro real devolveria 1)';
  }
  // b220.2 (Codex): UMA linha nao PROVA que filtrou.
  //
  // Se a conta tem poucas notas — ou se `tipo=1` deixa so uma — a consulta
  // SEM filtro nenhum tambem devolve 1, e por acaso a certa. Nao da pra
  // distinguir isso de um filtro que funcionou, entao nao afirmo.
  if (bateu) {
    // b220.3 (Codex): base AUSENTE tambem e duvida.
    //
    // Eu tratava `undefined` como se fosse conta cheia — mas base ausente
    // significa que a medicao falhou, entao nao sei se a consulta sem
    // filtro tambem devolveria 1. Afirmar FUNCIONA ali e chutar a favor.
    if (totalDaConta > 1) return 'FUNCIONA';
    if (totalDaConta === 1) {
      return 'PROVAVEL (veio 1 e e a certa, mas a consulta base tambem devolve 1 — '
        + 'teste com uma chave de conta cheia pra confirmar)';
    }
    return 'PROVAVEL (veio 1 e e a certa, mas nao consegui medir a consulta base — '
      + 'rode de novo pra confirmar)';
  }
  // com UMA linha e o detalhe falhando, nao sei dizer se era a certa
  if (detalheFalhou) return 'erro (detalhe da NF falhou)';
...
module.exports = function registrarRotasDebug(app, deps) {
  ...
  // b220 - O /nfe ACEITA FILTRO POR CHAVE?
  //
  ...
  //
  // A chave e UNICA. Se a API filtrar por ela, cai por terra tudo que
  // construimos hoje pra contornar numero ambiguo: escada de desempate,
  // escolha manual, filtro de serie. Vale medir antes de decidir.
  //
  // ⚠️ ESTA ROTA JA FOI REESCRITA. As 6 rodadas de revisao deixaram uma
  // cascata de ternarios ilegivel e um custo que ninguem tinha somado: ate
  // 24 chamadas ao Bling por execucao. Reescrita com o veredito em funcao
  // nomeada, teto de tempo e teto de chamadas.
  //
  ...
  app.get('/api/debug/testar-chave', (req, res, next) => {
    ...
    // b220: TETOS. Sem eles a rota fazia ate 24 chamadas e ~10s de pausa —
    // podia estourar o tempo do proxy e queimar cota do Bling a toa.
    const INICIO = Date.now();
    const TETO_MS = 20000;
    const MAX_DETALHES_POR_SONDA = 2;
    ...
    const formas = [
      ...
      // CONTROLE: a busca por numero, que ja funciona. Prova que a nota
      // existe e a conta esta certa — um FUNCIONA aqui nao diz nada sobre
      // filtrar por chave.
      { nome: 'numero (CONTROLE - nao e filtro por chave)', controle: true,
        url: `?limite=5&pagina=1&tipo=1&numero=${chave.substr(25, 9).replace(/^0+/, '')}` },
    ];

    // b220.2 (Codex): quantas notas a consulta BASE devolve?
    //
    // Se ela ja devolve 1, entao "veio 1 e e a certa" nao prova filtro
    // nenhum — pode ser a unica nota que existe. Uma chamada pra saber
    // isso vale mais que um FUNCIONA falso.
    // b220.5 (Codex): duas bases, porque as sondas nao sao iguais — duas
    // vao SEM `tipo=1` e uma COM. Comparar uma sonda sem tipo contra uma
    // base com tipo nao vale: `tipo=1` pode deixar 1 nota enquanto a
    // consulta sem tipo tem varias, e um filtro que funcionou sairia
    // rebaixado pra PROVAVEL sem motivo.
    const base = {};
    for (const [nome, sufixo] of [['semTipo', ''], ['comTipo', '&tipo=1']]) {
      if (Date.now() - INICIO > TETO_MS) break;
      try {
        const rb = await Promise.race([
          chamarBling('https://api.bling.com.br/Api/v3/nfe?limite=5&pagina=1' + sufixo),
          new Promise((ok) => setTimeout(() => ok(null), 6000)),
        ]);
        if (rb && rb.ok && Array.isArray(rb.data?.data)) base[nome] = rb.data.data.length;
        await sleep(400);
      } catch (e) { /* sem a base, o veredito vira PROVAVEL — que e honesto */ }
    }

    const saida = [];
    for (const f of formas) {
      if (Date.now() - INICIO > TETO_MS) {
        saida.push({ forma: f.nome, controle: f.controle || undefined,
          veredito: 'erro (nao deu tempo)', erro: 'teto de ' + (TETO_MS / 1000) + 's' });
        continue;
      }
      try {
        // b220.1 (Codex): o teto tem que valer DURANTE a chamada.
        //
        // Eu so checava ANTES do await — mas `chamarBling` usa timeout de
        // 30s e ainda tenta de novo depois de 401/429. So a primeira sonda
        // ja podia passar dos meus 20s, e o teto virava enfeite.
        //
        // `Promise.race` corta de verdade: o que vier primeiro manda.
        // b220.2 (Codex): o prazo e o QUE SOBRA, sem piso.
        //
        // O `Math.max(1000, ...)` deixava a chamada passar do teto quando
        // restava menos de 1s — e o detalhe repetia isso depois de mais
        // 400ms de pausa. O teto anunciado virava aproximado.
        const restante = () => TETO_MS - (Date.now() - INICIO);
        const r = await Promise.race([
          chamarBling('https://api.bling.com.br/Api/v3/nfe' + f.url),
          new Promise((ok) => setTimeout(() => ok({ ok: false, status: 408,
            error: 'estourou o teto da rota' }), restante())),
        ]);
        // b220.4 (Codex): 200 com corpo estranho NAO e "vazio".
        //
        // Eu virava lista vazia, e vazio significa "a API respondeu e nao
        // filtrou" — uma conclusao. Mas se o corpo veio malformado eu nao
        // sei NADA: pode ter filtrado e eu nao entendi a resposta.
        const corpoOk = r.ok && Array.isArray(r.data?.data);
        const lista = corpoOk ? r.data.data : [];
        if (r.ok && !corpoOk) {
          saida.push({
            forma: f.nome,
            ...
            http_ok: true,
            status: r.status || null,
            veredito: 'erro (resposta sem lista)',
            erro: 'HTTP 200 mas `data.data` nao veio como array',
          });
          await sleep(400);
          continue;
        }

        let achada = lista.find((nf) =>
          String(nf.chaveAcesso || '').replace(/\D/g, '') === chave) || null;
        let confirmadaPeloDetalhe = false;
        let detalheFalhou = false;

        // a listagem pode OMITIR `chaveAcesso` (documentado no b166.4), e ai
        // so o detalhe da nota resolve. Percorro as sem chave, ate o teto.
        // b220.1 (Codex): com mais de uma linha o veredito ja esta dado —
        // chave e unica, entao o filtro foi ignorado, seja qual for a nota.
        // Buscar detalhe ali gasta 2 chamadas e ~1s pra confirmar o que ja
        // sei. So vale quando ha UMA linha e a duvida e real.
        const valeODetalhe = f.controle || lista.length === 1;
        if (!achada && lista.length && valeODetalhe) {
          const semChave = lista.filter((nf) => !nf.chaveAcesso)
            .slice(0, MAX_DETALHES_POR_SONDA);
          for (const cand of semChave) {
            if (Date.now() - INICIO > TETO_MS) break;
            await sleep(400);
            const det = await Promise.race([
              buscarNFePorId(cand.id),
              new Promise((ok) => setTimeout(() => ok({ ok: false, status: 408 }),
                TETO_MS - (Date.now() - INICIO))),
            ]);
            // b220.5 (Codex): 200 sem corpo usavel tambem e falha — quarta vez
            // que "nao sei" virava "nao funciona" nesta rota, agora no detalhe.
            const nfd = (det.ok && det.data && typeof det.data.data === 'object') ? det.data.data : null;
            if (!det.ok || !nfd) { detalheFalhou = true; continue; }
        ...
        saida.push({
          ...
          com_a_chave_certa: achada ? 1 : 0,
          veredito: vereditoDaSonda({
            ehControle: !!f.controle,
            httpOk: !!r.ok,
            lista: lista.length,
            bateu: !!achada,
            detalheFalhou,
            // a base que combina com ESTA sonda
            totalDaConta: f.url.includes('tipo=1') ? base.comTipo : base.semTipo,
          }),
          confirmada_pelo_detalhe: confirmadaPeloDetalhe || undefined,
          id: achada ? achada.id : null,
        });
        await sleep(400);
      } catch (e) {
        // excecao tambem e sonda que falhou: sem `veredito` ela ficava fora
        // da contagem e a conclusao saia categorica sem base
        saida.push({
          forma: f.nome,
          controle: f.controle || undefined,
    ...
    // b220.3 (Codex): PROVAVEL nao e sucesso, mas tambem NAO e recusa.
    // Sem isto a conclusao dizia "nao da pra filtrar por chave" tendo uma
    // sonda que achou a nota — o oposto do que o dado sugere.
    const provaveis = deChave.filter((x) => String(x.veredito || '').startsWith('PROVAVEL'));
    const controleOk = saida.some((x) => x.controle && x.veredito === 'FUNCIONA');
    const falharamPorAcaso = deChave.filter(sondaFalhouPorAcaso);

    res.json({
      ok: true,
      conclusao: venceu
        ? 'DA PRA FILTRAR POR CHAVE usando `' + venceu.forma + '` — vale trocar a busca'
        : (falharamPorAcaso.length
          ? 'INCONCLUSIVO: ' + falharamPorAcaso.length + ' sonda(s) falharam por erro '
            + 'passageiro (5xx, 429, timeout ou rede). Rode de novo'
          : (provaveis.length
            ? 'INCONCLUSIVO: `' + provaveis[0].forma + '` achou a nota, mas nao da pra '
              + 'provar que foi o filtro (' + provaveis[0].veredito.replace(/^PROVAVEL \(/, '').replace(/\)$/, '')
              + ')'
          : (controleOk
      ...
      // b220.2: quantas notas a consulta SEM filtro de chave devolve —
      // se for 1, um "veio 1 e e a certa" nao prova nada
      base_sem_filtro: base,
      testes: saida,
      leia: 'Procure `veredito: "FUNCIONA"` numa linha SEM `controle: true` — essa '
        + 'forma acha a nota pela CHAVE, que e unica, e da pra trocar toda a busca '
        + 'por numero por uma chamada exata. "ignorou o filtro" = a API devolveu '
        + 'notas quaisquer. `confirmada_pelo_detalhe` = a lista nao trazia a chave '
        + 'e eu fui buscar na nota completa.',
    });
  });
...
// b220.2: expostas pro teste usar as de verdade
module.exports.vereditoDaSonda = vereditoDaSonda;
module.exports.sondaFalhouPorAcaso = sondaFalhouPorAcaso;
