...
module.exports = function registrarRotasDebug(app, deps) {
  ...
  app.get('/api/debug/capturar-agora', (req, res, next) => {
    ...
    try {
      ...
      const r = await forcarCaptura();
      return res.json({
        ok: true,
        resultado: r || null,
        estado: capturaEstado ? capturaEstado() : null,
        leia: 'Rodou agora, pulando o intervalo de 1h. Se `gravadas` vier 0 e `erro` vier nulo, '
          + 'nao havia nada novo — a captura usa upsert, entao regravar o mesmo nao conta.',
      });
    } catch (e) {
