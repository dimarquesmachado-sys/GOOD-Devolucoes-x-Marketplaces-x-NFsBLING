  ...
  app.get('/api/debug/testar-chave', (req, res, next) => {
    ...
    try {
      const rb = await Promise.race([
        chamarBling('https://api.bling.com.br/Api/v3/nfe?limite=5&pagina=1&tipo=1'),
        new Promise((ok) => setTimeout(() => ok(null), 8000)),
      ]);
      if (rb && rb.ok && Array.isArray(rb.data?.data)) totalDaConta = rb.data.data.length;
      await sleep(400);
    } catch (e) { /* sem isso, o veredito so nao usa o refinamento */ }
      ...
      try {
        ...
        if (!achada && lista.length && valeODetalhe) {
          ...
          for (const cand of semChave) {
            ...
            if (!det.ok) { detalheFalhou = true; continue; }
