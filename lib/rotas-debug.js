...
module.exports = function registrarRotasDebug(app, deps) {
  ...
  app.get('/api/debug/testar-chave', (req, res, next) => {
    ...
    for (const f of formas) {
      try {
        ...
        saida.push({
          ...
          // b219.6 (Codex): mais de UMA linha ja prova que o filtro foi
          // ignorado — a chave e unica, entao nao importa quais notas sao.
          // A falha do detalhe so obscurece o caso de UMA linha, onde eu
          // realmente nao sei se era a nota certa.
          veredito: !r.ok ? 'erro'
            : (!f.controle && lista.length > 1) ? 'ignorou o filtro (vieram '
    ...
    // b219.6: uma regra so pro que e passageiro — 4xx e recusa definitiva,
    // MENOS o 429 (fila) e o 408 (timeout), que pedem nova tentativa.
    const ehPassageiro = (x) => {
      if (!(x.veredito || '').startsWith('erro') && !x.erro) return false;
      const st = Number(x.status) || 0;
      if (st === 429 || st === 408) return true;          // fila / timeout
      return !(st >= 400 && st < 500);                     // 5xx, rede, excecao
    };
    ...
    res.json({
      ...
      conclusao: venceu
        ...
        // b219.6 (Codex): 429 e 4xx mas NAO e recusa — e limite de taxa, e
        // some sozinho. Trata-lo como "o parametro nao existe" faria a rota
        // concluir errado num dia de fila no Bling.
        : (deChave.some((x) => ehPassageiro(x))
          ? 'INCONCLUSIVO: ' + deChave.filter((x) => ehPassageiro(x)).length
