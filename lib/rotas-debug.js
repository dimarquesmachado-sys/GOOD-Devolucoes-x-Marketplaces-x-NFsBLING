...
module.exports = function registrarRotasDebug(app, deps) {
   ...
   */
  function vereditoDaSonda({ ehControle, httpOk, lista, bateu, detalheFalhou }) {
  ...
  app.get('/api/debug/testar-chave', (req, res, next) => {
    ...
    for (const f of formas) {
      ...
      try {
        ...
        const restante = () => Math.max(1000, TETO_MS - (Date.now() - INICIO));
        ...
        if (!achada && lista.length && valeODetalhe) {
          ...
          for (const cand of semChave) {
            ...
            const det = await Promise.race([
              ...
              new Promise((ok) => setTimeout(() => ok({ ok: false, status: 408 }),
                Math.max(1000, TETO_MS - (Date.now() - INICIO)))),
            ]);
