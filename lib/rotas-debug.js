  ...
  app.get('/api/debug/testar-chave', (req, res, next) => {
    ...
    for (const f of formas) {
      try {
        ...
        if (!bateu.length && lista.length && lista.length <= 5) {
          ...
          if (semChave.length) {
            await sleep(400);
            const det = await buscarNFePorId(semChave[0].id);
            const nfd = (det.ok && det.data?.data) ? det.data.data : null;
        ...
        saida.push({ forma: f.nome, erro: String(e.message || e).slice(0, 120) });
      }
    }
    ...
    res.json({
      ...
      conclusao: venceu
        ...
        : (deChave.some((x) => x.veredito === 'erro' || x.erro)
          ? 'INCONCLUSIVO: ' + deChave.filter((x) => x.veredito === 'erro' || x.erro).length
            + ' sonda(s) de chave falharam por erro da API. Rode de novo antes de concluir'
          : (controleOk
            ? 'NAO da pra filtrar por chave. A nota existe (o controle achou por numero) '
              + 'e todas as grafias responderam sem erro — nenhuma e aceita'
