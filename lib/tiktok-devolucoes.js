...
function normalizar(d, empresa) {
  ...
  return {
    ...
    // b184.4 (Codex): a NF tem que atravessar. Quem consome isto (a captura,
    // e depois o painel de estornadas) usa a NF pra duas coisas: casar o
    // registro quando o estoquista bipa a nota, e calcular o prazo de
    // cancelamento a partir da DATA DE EMISSAO, que mora na chave. Sem ela,
    // o prazo cai na data da devolucao e pode sugerir cancelar uma nota ja
    // intempestiva.
    nf_numero: d.nf_numero || d.nf || null,
    nf_chave: d.nf_chave || d.chave_nota || null,
...
function acharNaLista(lista, codigo, empresa) {
  ...
  const limpo = (x) => String(x || '').replace(/[^0-9A-Za-z]/g, '').toUpperCase();

  // b180 (Codex): um PEDIDO pode ter varias solicitacoes — o cliente abre,
  // cancela, abre de novo. Um pedido da Girassol (585234469423907917) teve
  // TRES em sequencia. Casar por pedido e devolver "a primeira que aparecer"
  // e sorte: podia entregar a cancelada em vez da que esta valendo.
  //
  // Entao: identificador ESPECIFICO (rastreio, id da devolucao) casa direto,
  // porque so existe um. O PEDIDO junta os candidatos e escolhe.
  const especificas = [];
  const porPedido = [];

  for (const d of lista) {
    if (!d) continue;
    const especificos = [d.rastreio, d.return_tracking_number, d.tracking, d.id, d.return_id];
    const pedidos = [d.pedido, d.order_id];
    if (especificos.some((x) => x && limpo(x) === alvo)) especificas.push(d);
    else if (pedidos.some((x) => x && limpo(x) === alvo)) porPedido.push(d);
  }

  // b180.3: os pacotes IRMAOS valem nos DOIS caminhos.
  //
  // Bipar o rastreio casa direto (o codigo e unico), mas o estoquista
  // continua precisando saber que ha outra caixa a caminho — senao ve
  // metade dos itens do pedido e marca divergencia.
  function comIrmaos(d) {
    const r = normalizar(d, empresa);
    const doPedido = lista.filter((x) => x && (
      (x.pedido && d.pedido && limpo(x.pedido) === limpo(d.pedido))
      || (x.order_id && d.order_id && limpo(x.order_id) === limpo(d.order_id))
    ));
    const vivas = doPedido.filter((x) => !/CANCEL/i.test(String(x.status || x.return_status || '')));
    const temElos = doPedido.some((x) => x.pre_return_id || x.next_return_id);

    if (!temElos && vivas.length > 1) {
      r.irmas = vivas.length;
      r.valor_total_pedido = Number(vivas.reduce((t, x) => t + (Number(x.valor || x.refund_amount) || 0), 0).toFixed(2));
      r.pacotes = vivas.map((x) => {
        const n = normalizar(x, empresa);
        return {
          id: n.id, rastreio: n.rastreio, transportadora: n.transportadora,
          status: n.status, valor: n.valor, itens: n.itens,
          esta: n.id === r.id,
        };
      });
      r.aviso = 'Este pedido tem ' + vivas.length + ' pacotes de devolucao (o TikTok abre um por item). '
        + 'Somados: R$ ' + r.valor_total_pedido.toFixed(2) + '. Confira o que vem NESTE.';
    }
    return r;
  }

  if (especificas.length) return comIrmaos(especificas[0]);
  if (!porPedido.length) return null;
  if (porPedido.length === 1) return normalizar(porPedido[0], empresa);


  // b180.2: VARIAS no mesmo pedido tem DUAS causas diferentes, e so uma
  // delas justifica escolher uma so.
  //
  //   (a) CADEIA: o cliente abre, cancela, abre de novo. Os elos aparecem
  //       em pre_return_id / next_return_id, e as antigas ficam CANCELADAS.
  //       So a ultima vale.
  //
  //   (b) UMA POR ITEM: o TikTok abre uma solicitacao para cada item da
  //       nota. Medido no pedido 585110624384091852 da Girassol — nota com
  //       DUAS linhas de R$ 59,90, dois reembolsos de R$ 57,40, ambos
  //       CONCLUIDOS e sem elos. As DUAS valem, e o prejuizo e a soma.
  //
  // Descartar uma no caso (b) esconderia metade do prejuizo. Entao: se nao
  // ha elos e mais de uma esta viva, todas contam — mostro a mais recente
  // mas aviso que sao irmas, com o total somado.
  const temElos = porPedido.some((d) => d.pre_return_id || d.next_return_id);
  const cancelada = (d) => /CANCEL/i.test(String(d.status || d.return_status || ''));
  const vivas = porPedido.filter((d) => !cancelada(d));
  const candidatas = vivas.length ? vivas : porPedido;

  candidatas.sort((a, b) => Number(b.atualizado_em || b.criado_em || 0) - Number(a.atualizado_em || a.criado_em || 0));

  const escolhida = comIrmaos(candidatas[0]);
  if (porPedido.length > 1) {
    escolhida.varias_no_pedido = porPedido.length;

    if (!temElos && vivas.length > 1) {
      // caso (b): uma por item da nota — as irmas contam junto
      const soma = vivas.reduce((t, d) => t + (Number(d.valor || d.refund_amount) || 0), 0);
      escolhida.irmas = vivas.length;
      escolhida.valor_total_pedido = Number(soma.toFixed(2));

      // b180.3 (Codex): mostrar so os itens de UMA esconde o resto. Se o
      // pedido tem 2 caixas, o estoquista precisa saber o que vem em CADA
      // uma — senao, ao bipar a 1a e ver metade dos itens, marca divergencia.
      //
      // E o dono pediu isso explicitamente: "seria legal saber o q ta vindo
      // naquela devolucao, parcial ou nao, e mostrar isso na triagem".
      escolhida.pacotes = vivas.map((d) => {
        const n = normalizar(d, empresa);
        return {
          id: n.id,
          rastreio: n.rastreio,
          ...
          valor: n.valor,
          itens: n.itens,
          esta: n.id === escolhida.id,   // qual deles e o que foi bipado
        };
      });
      escolhida.aviso = 'Este pedido tem ' + vivas.length + ' solicitacoes ATIVAS (o TikTok abre uma por item). '
        + 'Somadas: R$ ' + soma.toFixed(2) + '. Todas valem — nao e duplicata.';
    } else {
      // caso (a): cadeia — so a ultima vale
      escolhida.aviso = 'Este pedido tem ' + porPedido.length + ' solicitacoes em sequencia '
        + '(o cliente reabriu); mostrando a que esta valendo.';
    }
  }
  return escolhida;
}
