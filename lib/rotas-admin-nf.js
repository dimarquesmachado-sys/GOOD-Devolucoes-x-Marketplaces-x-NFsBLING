...
module.exports = function registrarRotasAdminNF(app, deps) {
  ...
  app.get('/api/produto/imagem/:id', async (req, res) => {
    ...
    try {
      ...
      let url = null;
      if (!id) {
        const rB = await chamarBling(`https://api.bling.com.br/Api/v3/produtos?codigo=${encodeURIComponent(chave)}&limite=1`);
        const item = (rB.ok && rB.data && rB.data.data && rB.data.data[0]) || null;
