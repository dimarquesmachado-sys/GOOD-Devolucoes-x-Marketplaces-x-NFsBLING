...
module.exports = function registrarRotasAdminNF(app, deps) {
  ...
  app.get('/api/produto/imagem/:id', async (req, res) => {
    ...
    try {
      ...
      if (!id) {
        const rB = await chamarBling(`https://api.bling.com.br/Api/v3/produtos?codigo=${encodeURIComponent(chave)}&limite=5`);
        const lista = (rB.ok && rB.data && rB.data.data) || [];
        const alvo = lista.find(p => String(p.codigo || '').trim().toUpperCase() === chave.toUpperCase()) || lista[0];
        id = alvo && alvo.id ? alvo.id : null;
      }
      if (!id) { IMG_CACHE.set(chave, null); return res.json({ ok: true, id: chave, imagem: null }); }
      const r = await chamarBling(`https://api.bling.com.br/Api/v3/produtos/${encodeURIComponent(id)}`);
      const det = (r.ok && r.data && r.data.data) || null;
