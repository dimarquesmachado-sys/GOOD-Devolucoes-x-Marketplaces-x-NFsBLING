...
module.exports = function registrarRotasAdminNF(app, deps) {
  ...
  app.get('/api/produto/imagem/:id', async (req, res) => {
    ...
    try {
      ...
      let id = null;
      let url = null;
      const rL = await chamarBling(`https://api.bling.com.br/Api/v3/produtos?codigo=${encodeURIComponent(chave)}&limite=3`);
      const achados = (rL.ok && rL.data?.data) || [];
      const porCodigo = achados.find(p => String(p.codigo || '').toUpperCase() === chave.toUpperCase()) || achados[0] || null;
      if (porCodigo) { url = primeiraImagem(porCodigo); id = porCodigo.id || null; }
      if (!id && /^\d{12,14}$/.test(chave)) {
        const rE = await chamarBling(`https://api.bling.com.br/Api/v3/produtos?gtin=${encodeURIComponent(chave)}&limite=1`);
        const porEan = (rE.ok && rE.data?.data && rE.data.data[0]) || null;
        if (porEan) { url = url || primeiraImagem(porEan); id = porEan.id || null; }
      }
      if (!id && /^\d{6,}$/.test(chave)) id = chave;
      if (!url && id) {
        const r = await chamarBling(`https://api.bling.com.br/Api/v3/produtos/${encodeURIComponent(id)}`);
        url = primeiraImagem((r.ok && r.data && r.data.data) || null);
      }
      // so cacheia SUCESSO — falha passageira nao pode deixar o produto
      // sem foto pra sempre (licao do produtoDetalhe do checkout)
      if (url) IMG_CACHE.set(chave, url);
      res.json({ ok: true, id: chave, imagem: url });
    } catch (e) {
