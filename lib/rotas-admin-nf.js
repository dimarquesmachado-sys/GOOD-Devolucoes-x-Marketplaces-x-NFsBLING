...
module.exports = function registrarRotasAdminNF(app, deps) {
  ...
  app.get('/api/produto/imagem/:id', async (req, res) => {
    ...
    try {
      ...
      //
      // ⚠️ E ESTA TENTATIVA CUSTOU CARO NA PRIMEIRA VERSAO: a lista pede
      // foto de ate 12 produtos DE UMA VEZ, cada um com ate 4 tentativas =
      // ~50 chamadas simultaneas. Isso estourou a cota e o Bling recusou
      // TODAS — inclusive as que antes funcionavam. O dono viu "sumiram
      // todas as imagens", pior que o problema original.
      //
      // 📌 Entao ela so roda quando a chamada TEM ORCAMENTO: se o porteiro
      // ja esta segurando a conta, desisto da foto. Enfeite nao pode
      // atropelar bipagem.
      // v4.9x (review do Codex) - o orcamento era gasto ANTES de checar se o
      // `pai` derivado da chave era usavel. Uma chave como `A-B` consumia a
      // cota mesmo sem chegar a consultar o Bling (o `pai.length >= 3`
      // barrava depois), e seis chaves assim no mesmo minuto esgotavam o
      // teto e derrubavam a foto do pai de itens validos que vinham a
      // seguir. Agora deriva e valida o `pai` PRIMEIRO; so gasta orcamento
      // no instante em que a chamada ao Bling de fato vai acontecer.
      if (!url && /[-_]/.test(chave)) {
        const pai = chave.replace(/[-_][^-_]+$/, '');
        if (pai && pai !== chave && pai.length >= 3 && podeGastarNaFoto()) {
          const porPai = await listar(`codigo=${encodeURIComponent(pai)}`);
          const prodPai = porPai.find((x) => String(x.codigo || '').toUpperCase() === pai.toUpperCase())
            || porPai[0] || null;
          if (prodPai) {
            const u = primeiraImagem(prodPai);
            if (u) { url = u; via = 'pai_da_variacao'; id = id || prodPai.id || null; }
          }
        }
      }
...
app.post('/api/admin/full-vincular/:id', requerAdmin, async (req, res) => {
  ...
  try {
    ...
    for (const cand of candidatos.slice(0, 3)) {
      ...
      const sNF = String(nf.serie || '').trim().replace(/^0+/, '')
        || String(nf.chaveAcesso || '').replace(/\D/g, '').substr(22, 3).replace(/^0+/, '');
