...
module.exports = function registrarRotasAdminNF(app, deps) {
  ...
  app.get('/api/produto/imagem/:id', async (req, res) => {
    ...
    if (typeof deps.fotoDoIndice === 'function') {
      var doIndice = deps.fotoDoIndice(chave);
      if (doIndice) {
        IMG_CACHE.set(chave, doIndice);
      ...
      if (typeof deps.fotoDoIndice === 'function') {
        for (let tentativa = 0; tentativa < 2; tentativa++) {
          ...
          if (doIndicePriorizado) {
            IMG_CACHE.set(chave, doIndicePriorizado);
    ...
    // - sem essa flag nada muda, o fallback completo continua igual pras
    // outras chamadas desta rota (lancar-defeito.js, busca.js).
    if (req.query.semBling) {
      return res.json({ ok: true, id: chave, imagem: null, via: 'sem_indice_sem_bling' });
    }
