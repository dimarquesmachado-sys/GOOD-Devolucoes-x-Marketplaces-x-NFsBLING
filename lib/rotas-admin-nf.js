...
module.exports = function registrarRotasAdminNF(app, deps) {
  ...
  app.get('/api/produto/imagem/:id', async (req, res) => {
    ...
    try {
      ...
      //
      // CASO REAL (11/09): varios defeitos na lista sem foto, e o padrao
      // saltou no print — `288-VAR`, `801s-BP`, `RA-45-GOLD-ASH`,
      // `KJDD-E-003-GOLD`. Todos VARIACOES.
      //
      ...
      // as 3 tentativas acima procuravam so por ela.
      //
      // Corto o sufixo no ultimo separador e tento o pai:
      //   288-VAR   -> 288
      //   801s-BP   -> 801s
      //   RA-45-GOLD-ASH -> RA-45-GOLD
      //
      // 📌 Uma tentativa so, e so quando as outras falharam: nao vale gastar
      // chamada do Bling pra foto, que e enfeite.
      if (!url && /[-_]/.test(chave)) {
