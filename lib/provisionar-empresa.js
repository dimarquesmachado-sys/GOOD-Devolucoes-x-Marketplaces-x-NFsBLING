...
async function provisionarEmpresa(chaveEmpresa, cliente) {
  ...
  for (const base of ESPERADAS) {
    ...
    const msg = String((t.error && t.error.message) || '');
    const cod = String((t.error && t.error.code) || '');
    if (t.error && (/does not exist|could not find the table/i.test(msg)
        || /42P01|PGRST205/i.test(cod))) {
      faltando.push(nome);
    }
  }
  if (faltando.length) {
