...
async function sondaDevolucoes(empresa, q) {
  ...
  if (out.coleta_enfileirada && String(q.esperar || '') !== '1') {
    out.ok = true;
    out.coleta_pendente = true;
    out.aviso = 'a coleta foi ACEITA mas ainda esta rodando: a lista abaixo e do estado ANTERIOR. '
      + 'Chame de novo em alguns minutos, ou use &esperar=1 pra bloquear ate terminar.';
    out.ultima_coleta = ult || null;
    out.cru = r.corpo;
    return out;
  }

  if (ult && (ult.status === 'falhou' || ult.ok === false)) {
    out.ok = false;
