...
const LOJA_POR_EMPRESA = { good: 'good', ambtotal: 'amb' };
...
async function sondaDevolucoes(empresa, q) {
  ...
  if (String(q.coletar) === '1') {
    const c = await chamarMoverPedidos('/tiktok/devolucoes-coletar', { loja, dias: q.dias }, { timeoutMs: 90000 });
    ...
    if (!c.corpo || c.corpo.loja !== loja) {
      out.ok = false;
      out.erro = (c.corpo && c.corpo.loja)
        ? 'coleta respondeu a loja "' + c.corpo.loja + '" quando pedi "' + loja + '"'
        : 'coleta respondeu SEM identificar a loja (versao antiga/incompativel?) quando pedi "' + loja + '"';
      return out;
    }
  }
...
module.exports = { sondaDevolucoes, chamarMoverPedidos, lojaDaEmpresa, configPonte };
