...
// b260 - recursos cujo 403 ja se provou PERMANENTE (renovar nao resolveu).
// ⚠️ Guardo o CAMINHO sem querystring: `/shipments/123` e `/shipments/456`
// sao recursos diferentes, mas se o 403 e de escopo, os dois vao falhar —
// e eu quero descobrir isso uma vez por rota, nao uma vez por id.
const ML_403_PERMANENTE = new Set();
const ML_403_MAX = 200;   // teto, pra memoria nao crescer sem limite
...
async function chamarML(url, headersExtras = {}) {
  ...
  try {
    ...
    // b260 - a lista dos recursos onde o 403 JA se provou permanente.
    // Curta e por processo: reinicio limpa, e um recurso que volta a
    // funcionar sai dela no proximo boot.
    const chaveRec = String(url).split('?')[0];
    if (st === 403 && ML_403_PERMANENTE.has(chaveRec)) {
      console.warn(`[ML] 403 conhecido em ${chaveRec} — NAO renovo (ja provei que nao adianta)`);
      ...
      if (tokenLeitor.politicaDe('good', 'ml') === 'remoto') {
        try {
          ...
          return { ok: true, data: r.data, status: r.status };
            ...
            if (st === 403 && ML_403_PERMANENTE.size < ML_403_MAX) {
              ML_403_PERMANENTE.add(chaveRec);
              console.warn(`[ML] 403 PERMANENTE em ${chaveRec}: renovei e continuou. `
                + 'Nao vou mais renovar por causa dele (era escopo/permissao, nao token). '
                + `Total marcado: ${ML_403_PERMANENTE.size}`);
            }
...
function diagnostico403() {
  return {
    recursos_com_403_permanente: [...ML_403_PERMANENTE],
    total: ML_403_PERMANENTE.size,
    _nota: 'renovar token NAO resolve 403 destes — e escopo/permissao. '
      + 'A lista zera no reinicio; recurso que voltar a funcionar sai dela.',
  };
}
