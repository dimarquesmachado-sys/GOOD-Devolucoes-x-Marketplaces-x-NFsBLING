...
async function chamarML(url, headersExtras = {}) {
  ...
  try {
    ...
    // b267 - ⚠️ 403 DE ROTA JA CONHECIDA NAO INVALIDA O CACHE.
    //
    ...
    //
    // O limitador do b260 ja impedia a RENOVACAO a toa. Mas eu continuava
    // invalidando o CACHE a cada 403 — 30 releituras do dono por nada, e o
    // veredito travado em "retry falhou 15x".
    //
    // 📌 A regra do contrato v9 vale pro 403 DESCONHECIDO: invalidar e
    // barato, e se for vencimento a releitura corrige. O que muda e a rota
    // que JA se provou — ali a releitura devolve o mesmo token.
    const rota403Conhecida = st === 403 && !podeRenovarPor403(url);

    if ((st === 401 || st === 403) && !rota403Conhecida) {
