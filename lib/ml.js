...
function podeRenovarPor403(url) {
  const ultima = ML_ULTIMA_RENOV_POR_ROTA.get(rotaDe(url)) || 0;
  return Date.now() - ultima >= ML_JANELA_RENOV_MS;
}

/**
 * b260.1 (Codex, P2) - ⚠️ O RELOGIO SO ANDA SE O REFRESH FOI GASTO.
 *
 * Antes, `podeRenovarPor403` consultava E registrava na mesma chamada.
 * Entao a janela comecava mesmo quando:
 *   - a renovacao OAuth falhava (nenhum refresh consumido)
 *   - a politica era `remoto` (nao ha renovacao local nenhuma)
 *
 * Nos dois casos eu bloqueava a rota por 10 min sem ter gasto nada — e em
 * `remoto` isso e pior: a guarda saia ANTES da invalidacao do cache, entao
 * o token velho do dono seguia sendo usado ate o TTL.
 *
 * Agora sao duas coisas: perguntar e registrar.
 */
function registrarRenovacaoPor403(url) {
  ML_ULTIMA_RENOV_POR_ROTA.set(rotaDe(url), Date.now());
}
...
async function chamarML(url, headersExtras = {}) {
  ...
  try {
    ...
    // b267 - ⚠️ 403 DE ROTA JA CONHECIDA NAO GASTA OUTRO REFRESH.
    //
    // MEDIDO EM 6H DE OPERACAO REAL (10/09, /health): 30 respostas 403 do
    // ML e ZERO 401. Trinta invalidacoes, 15 retries, TODOS falharam.
    //
    // ⚠️ 403 SEM NENHUM 401 JUNTO NAO E TOKEN VENCIDO. Token vencido daria
    // 401 tambem, e em TODAS as rotas — nao numa so. O painel mostra 1 rota
    // concentrando tudo: as duas chamadas de `/shipments/{id}` do ciclo de
    // datas de entrega, que roda a cada 5 min. O ML restringe esse
    // endpoint (o envio e do COMPRADOR); nenhum token resolve.
    //
    // O limitador do b260 ja impedia a RENOVACAO a toa. A minha 1a versao
    // deste conserto tambem cortava a INVALIDACAO do cache pra rota ja
    // conhecida, achando que "invalidar sem renovar" era desperdicio igual.
    //
    // b267.1 (Codex, P2) - ⚠️ NAO E. O Codex apontou: se o dono ROTACIONAR o
    // token durante a janela de 10 min (outro processo renovou, ou o token
    // simplesmente venceu de verdade), pular a invalidacao mantinha o token
    // VELHO no cache ate o TTL de 5 min OU o fim da janela — atrasando uma
    // recuperacao que a proxima chamada teria resolvido na hora.
    //
    // 📌 A regra do contrato v9 vale pro 401/403: invalidar e barato (so
    // limpa cache local, nao gasta refresh nem chama o ML), e se for
    // vencimento a releitura corrige. A rota JA conhecida (b267) so deixa
    // de gastar REFRESH — nao deixa mais de invalidar (b267.1 abaixo).
    const rota403Conhecida = st === 403 && !podeRenovarPor403(url);
    ...
    if (st === 401 || st === 403) {
      ...
      //
      // b267.1 (Codex, P2) - ⚠️ SEMPRE, MESMO NA ROTA JA CONHECIDA. O b267
      // tinha posto isto atras de `!rota403Conhecida`, junto com o retry —
      // mas invalidar e BARATO (so limpa o cache local; nao gasta refresh
      // nem chama o ML). Pular invalidacao na rota conhecida so servia pra
      // deixar o token cacheado (possivelmente ja rotacionado pelo dono)
      // vivo ate o TTL de 5 min OU o fim da janela de 10 min — atrasando
      // uma recuperacao que a proxima chamada, de qualquer rota, poderia
      // ter resolvido na hora. O que precisa ficar condicional e so o
      // GASTO de refresh, mais abaixo.
      ...
      // Isto roda mesmo em rota ja conhecida: `remoto` nao gasta refresh
      // local nenhum (o unico recurso que a janela do b260 protege), entao
      // nao ha motivo pra negar a releitura do dono aqui.
      if (tokenLeitor.politicaDe('good', 'ml') === 'remoto') {
        try {
          ...
          // logo morta pra sempre — o teste que devia acusar isso so olhava
          // a linha ANTERIOR, e havia um bloco de comentario no meio.
          tokenLeitor.anotarRetry('good', 'ml', true, st);
          return { ok: true, data: r.data, status: r.status };
        } catch (err2) {
            tokenLeitor.anotarRetry('good', 'ml', false, st);
      ...
      // b260.1 (Codex, P2) - ⚠️ A GUARDA VEM DEPOIS DA INVALIDACAO.
      //
      // Antes ela saia da funcao ANTES de invalidar o cache — entao, em
      // `remoto`, o token velho do dono seguia em uso ate o TTL de 5 min,
      // e a guarda tinha PIORADO o problema que veio consertar.
      //
      // Aqui em baixo, o cache ja foi invalidado e o retry em `remoto` ja
      // aconteceu (com token novo do dono). O que a guarda protege agora e
      // so a RENOVACAO LOCAL — que e a unica que gasta refresh.
      if (rota403Conhecida) {
      ...
      // ⚠️ e o relogio so anda SE A RENOVACAO ACONTECEU. `renovarTokenML`
      // devolve false quando a politica e `remoto`/`bloqueado` (nao gasta
      // refresh) ou quando o OAuth falha (nao consumiu nada). Marcar antes
      // bloquearia a rota por 10 min a troco de nada.
      const renovou = await renovarTokenML();
      if (renovou && st === 403) registrarRenovacaoPor403(url);
      if (renovou) {
        try {
          const r = await fazer();
          tokenLeitor.anotarRetry('good', 'ml', true, st);
