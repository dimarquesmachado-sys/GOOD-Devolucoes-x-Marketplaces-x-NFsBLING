...
module.exports = function registrarCicloDefeitos(app, deps) {
  ...
  app.post('/api/defeitos/:id/restaurar', requerLogin, async (req, res) => {
    ...
    try {
      const rI = await dbc.from(T_DEV).select('id, tipo, tipo_anterior, status, nf_numero, nf_chave, nf_id_bling').eq('id', req.params.id).limit(1).then(r => r).catch(() => ({ data: null, error: { message: 'erro' } }));
      let item = (rI.data || [])[0] || null;
      if (!item && rI.error) {
        const r2 = await dbc.from(T_DEV).select('id, tipo, status, nf_numero, nf_chave, nf_id_bling').eq('id', req.params.id).limit(1);
        item = (r2.data || [])[0] || null;
      }
      ...
      //
      // O sinal que separa os dois na GOOD: defeito lancado do ESTOQUE nao
      // tem NF nem cliente — e controle interno (server.js:4198-4203). Se ha
      // vestigio de NF, a linha nasceu de DEVOLUCAO e volta como 'problema'.
      const veioDeDevolucao = !!(item.nf_numero || item.nf_chave || item.nf_id_bling);
  ...
  app.get('/api/defeitos/lista', requerLogin, async (req, res) => {
    ...
    async function buscar(termo) {
      let sel = dbc.from(T_DEV)
        ...
        //
        // status.eq.problema fica no filtro de proposito: nao custa nada e
        // cobre registro antigo que porventura tenha sido gravado no molde
        // da AMB.
        ...
        .or('and(tipo.eq.problema,status.eq.concluido),tipo.eq.defeito_estoque,tipo.eq.recuperado,tipo.eq.descartado,tipo.eq.defeito_excluido')   // v4.62 - registro excluido some da lista
