...
module.exports = function registrarCicloDefeitos(app, deps) {
  ...
  app.get('/api/defeitos/lista', requerLogin, async (req, res) => {
    ...
    try {
      ...
      // seg4.4 (Codex) - a contagem tambem passava pelo limite compartilhado:
      // com o total acima de 300, as abas mostravam numero MENOR que a
      // propria lista exibida. Agora conta ABA POR ABA, cada uma com o seu
      // proprio limite — e o "excluido" fica de fora do somatorio de propos-
      // ito, porque a tela nao mostra contador pra ele.
      // b314 - ⚠️ A ABA DE EXCLUIDOS NUNCA FOI CONTADA.
      //
      // [stated 13/09] "não tá mostrando a contagem certa no botão
      // EXCLUÍDOS. já excluí 2, e mostra 0"
      //
      // O objeto tinha 3 abas e o laco varria 3 — `excluido` ficou de fora
      // desde sempre, entao o botao mostrava 0 mesmo com registros la.
      //
      // ⚠️ E ISSO E PIOR QUE COSMETICO: o dono exclui, ve 0, e conclui que
      // a exclusao nao funcionou — foi exatamente o que aconteceu.
      const contagem = { defeito: 0, recuperado: 0, descartado: 0, excluido: 0 };
      for (const aba of ['defeito', 'recuperado', 'descartado', 'excluido']) {
        const linhasDaAba = await buscar(termoContagem, aba, porPedido);
        contagem[aba] = linhasDaAba.filter(x => situacaoDe(x) === aba).length;
      }
