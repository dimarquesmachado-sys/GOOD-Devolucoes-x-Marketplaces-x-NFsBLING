...
module.exports = function registrarCicloDefeitos(app, deps) {
  ...
  app.post('/api/defeitos/:id/restaurar', requerLogin, async (req, res) => {
    ...
    try {
      ...
      if (!excluidoPorTipo && !excluidoPorStatus) {
  ...
  app.get('/api/defeitos/lista', requerLogin, async (req, res) => {
    ...
    // ela.
    const EXCLUIDOS = 'tipo.eq.defeito_excluido,and(tipo.eq.problema,status.eq.cancelled)';
    ...
    async function buscar(termo, estado, porPedido) {
      ...
      let sel = dbc.from(T_DEV)
        ...
        .select('id, produto_sku, produto_titulo, localizacao, defeito_qtd, produto_qtd, problema_descricao, problema_fotos, tipo, status, funcionario, nf_numero, created_at')
      ...
      // b310 - ⚠️ QUEM TEM MARCA DE EXCLUIDO SAI DAS LISTAS ATIVAS.
      //
      ...
      //
      // ⚠️ SO NAS ABAS ATIVAS: na aba de Excluidos ele PRECISA aparecer,
      // senao o dono nao consegue restaurar — e o texto da propria tela
      // promete que da ("de onde dá pra restaurar").
      if (estado !== 'excluido') {
        sel = sel.not('estado_atual', 'ilike', '%REGISTRO EXCLUIDO%');
      } else {
        sel = sel.ilike('estado_atual', '%REGISTRO EXCLUIDO%');
      }
      sel = sel
    ...
    try {
      ...
      const situacaoDe = (x) => {
        ...
        // ⚠️ b307 - e o marcador que a tabela ACEITA: `pendente` +
        // `estado_atual` de excluido.
        //
        // `cancelled` aparece no codigo mas a TABELA nao aceita (o dono
        // levou `violates check constraint "devolucoes_status_check"`).
        // Sem esta linha o registro sumiria de TODAS as abas — inclusive da
        // de Excluidos, e ele nao conseguiria restaurar.
        if (x.tipo === 'problema' && x.status === 'pendente'
          && /REGISTRO EXCLUIDO/.test(String(x.estado_atual || ''))) return 'excluido';
