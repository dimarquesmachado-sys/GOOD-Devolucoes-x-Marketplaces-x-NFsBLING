...
const { marcadoExcluido, MARCA_EXCLUIDO } = require('./defeito-excluido');
...
module.exports = function registrarCicloDefeitos(app, deps) {
  ...
  app.post('/api/defeitos/:id/restaurar', requerLogin, async (req, res) => {
    ...
    try {
      const rI = await dbc.from(T_DEV).select('id, tipo, tipo_anterior, status, estado_atual').eq('id', req.params.id).limit(1).then(r => r).catch(() => ({ data: null, error: { message: 'erro' } }));
      let item = (rI.data || [])[0] || null;
      if (!item && rI.error) {
        const r2 = await dbc.from(T_DEV).select('id, tipo, status, estado_atual').eq('id', req.params.id).limit(1);
        item = (r2.data || [])[0] || null;
      }
      if (!item) return res.status(404).json({ ok: false, erro: 'registro nao encontrado' });
      // b305 (revisao Codex #264) - a exclusao pode ter ficado marcada so
      // pelo `status` em vez do `tipo` (ver /excluir logo abaixo): quando a
      // tabela recusa gravar `tipo: 'defeito_excluido'` (lista fechada), o
      // /excluir grava `status: 'cancelled'` mantendo `tipo: 'problema'`.
      // O /restaurar precisa reconhecer os dois formatos — antes so olhava
      // o `tipo`, e uma exclusao-por-status nunca seria restaurada (o botao
      // dizia "nao esta excluido" pro proprio registro que a caixa
      // Excluidos mostra).
      const excluidoPorTipo = item.tipo === 'defeito_excluido';
      const excluidoPorStatus = item.tipo === 'problema' && String(item.status) === 'cancelled';
      // b311 (revisao Codex #269, P1) - TERCEIRO formato (b310): quando nem
      // `tipo` nem `status` aceitam gravar, o /excluir marca SO o
      // `estado_atual` — tipo e status ficam intocados, exatamente como
      // estavam antes da exclusao. Sem reconhecer isto aqui, o botao
      // Restaurar respondia 400 "nao esta excluido" pro proprio registro
      // que a aba Excluidos mostra.
      const excluidoPorMarca = !excluidoPorTipo && !excluidoPorStatus
        && MARCA_EXCLUIDO.test(String(item.estado_atual || ''));
      if (!excluidoPorTipo && !excluidoPorStatus && !excluidoPorMarca) {
        return res.status(400).json({ ok: false, erro: 'este registro nao esta excluido' });
      }

      let rU;
      if (excluidoPorStatus) {
        // o tipo nunca chegou a mudar (a gravacao foi recusada pelo check)
        // - restaurar e so devolver o status e limpar o aviso de exclusao.
        rU = await dbc.from(T_DEV).update({ status: 'concluido', estado_atual: null }).eq('id', req.params.id);
      } else if (excluidoPorMarca) {
        // idem: nem tipo nem status mudaram de verdade (ver comentario
        // acima) - mexer neles aqui e que INVENTARIA uma mudanca que nunca
        // aconteceu (ex.: um defeito_estoque marcado so pela estado_atual
        // viraria 'problema' de graca, pela logica de adivinhacao do bloco
        // de baixo, feita pra um cenario onde o tipo REALMENTE tinha virado
        // 'defeito_excluido'). Aqui basta apagar a marca.
        rU = await dbc.from(T_DEV).update({ estado_atual: null }).eq('id', req.params.id);
      } else {
        // Sem a coluna tipo_anterior, o fallback tinha que adivinhar se a
        // linha era devolucao de venda ou defeito de estoque manual (pelo
        // shipment_id sintetico `DEF-...`) pra saber se devolvia 'problema'
        // ou 'defeito_estoque'.
        //
        // b298 (revisao Codex #252) - a adivinhacao morreu: o PR #252 provou
        // que a tabela NUNCA aceitou `tipo: 'defeito_estoque'` (violates
        // check constraint "devolucoes_tipo_check") e trocou a GRAVACAO do
        // defeito manual pra `tipo: 'problema'`. 'problema' e hoje o UNICO
        // tipo que a tabela aceita de volta tambem — devolver
        // 'defeito_estoque' aqui levaria o mesmo 500 do banco, e o item
        // ficaria excluido pra sempre porque o /restaurar nunca teria
        // sucesso. A distincao devolucao-de-venda vs. defeito-de-estoque
        // continua existindo, so que agora fora do `tipo`: quem le a linha
        // (ex.: server.js, ehDefeitoDeEstoqueManual) olha o shipment_id
        // sintetico direto, que o soft delete/restore nunca apaga.
        const tipoVolta = item.tipo_anterior !== undefined && item.tipo_anterior !== null
          ? item.tipo_anterior
          : (String(item.status || '') === 'problema' ? null : 'problema');
        const camposR = { tipo: tipoVolta, estado_atual: null };
        rU = await dbc.from(T_DEV).update({ ...camposR, tipo_anterior: null }).eq('id', req.params.id);
  ...
  app.post('/api/defeitos/:id/excluir', requerLogin, async (req, res) => {
    ...
    if (!dbc) return erroSemBanco(res);
    try {
      const rI = await dbc.from(T_DEV).select('id, tipo, status, estado_atual, produto_sku').eq('id', req.params.id).limit(1);
      const item = (rI.data || [])[0] || null;
      if (!item) return res.status(404).json({ ok: false, erro: 'registro nao encontrado' });
      // b312 (revisao Codex #269, P2) - esta trava so olhava `tipo`. Um
      // registro excluido pelo fallback do estado_atual (b310) preserva o
      // tipo original, entao um RETRY (resposta perdida, duplo clique) nao
      // batia aqui, executava a exclusao de novo e sobrescrevia o motivo/
      // quem-excluiu original, alem de duplicar a entrada no historico.
      if (marcadoExcluido(item)) return res.json({ ok: true, ja_excluido: true });
      ...
      // b305 (revisao Codex #264) - ⚠️ o TIPO pode nao passar no check da
      // tabela. CASO REAL (13/09): o dono clicou "Excluir este registro" e
      // levou "new row violates check constraint devolucoes_tipo_check" —
      // `defeito_excluido` nao esta na lista fechada de `tipo`, igual
      // `defeito_estoque` ja tinha pegado antes (b298). O codigo LE esse
      // valor em varios lugares, mas a tabela nunca aceitou GRAVAR.
      //
      // Fallback: marca so pelo `status` ('cancelled'). A aba Excluidos, a
      // situacaoDe() (mais abaixo) e o /restaurar (acima) tambem reconhecem
      // esse formato, entao o soft delete continua reversivel.
      //
      // 📌 O certo e a tabela aceitar os tipos do ciclo — migracao em
      // producao, anotada. Isto e o que da pra fazer sem derrubar o dono.
      if (rU.error && /check constraint/.test(String(rU.error.message || ''))) {
        rU = await dbc.from(T_DEV).update({
          // ⚠️ b310 - SO O `estado_atual`, QUE E TEXTO LIVRE.
          //
          // [stated 13/09] tres erros seguidos de check, e todos meus:
          //   defeito_excluido (tipo)  -> tipo_check recusou
          //   cancelled (status)       -> status_check recusou
          //   pendente (status)        -> status_check recusou
          //
          // ⚠️ EU ESTAVA ADIVINHANDO valores de lista fechada, um por vez, e
          // cada tentativa custou uma rodada do dono. Parei.
          //
          // O `estado_atual` NAO tem check — e texto. Escrevo a marca la, e a
          // LISTA passa a filtrar por ela. Sem depender de nenhum valor que eu
          // nao consiga verificar daqui.
          estado_atual: camposExc.estado_atual,
        }).eq('id', req.params.id);
        if (!rU.error) {
          console.log('[DEFEITOS] excluido via status (o tipo nao passa no check)');
        }
      }
      if (rU.error) throw new Error(rU.error.message);
      try {
  ...
  app.get('/api/defeitos/lista', requerLogin, async (req, res) => {
    ...
    // seg4.2 (apontamento do Codex) - o LIMITE tem que vir DEPOIS do filtro
    // de aba, nao antes. Como estava, o .limit(300) cortava o conjunto
    // COMBINADO (ativos + recuperados + descartados + excluidos) e so
    // entao o estado era aplicado em JS: com historico grande, defeito
    // ativo antigo sumia da aba "Com Defeito" e as abas terminais vinham
    // furadas. Agora a condicao da aba entra na propria consulta.
    // Traduz a ABA em condicao de banco. O caso "recuperado/descartado
    // por PEDIDO" (historico antigo, quando o tipo da linha ainda nao era
    // marcado) nao da pra expressar em SQL sem join — por isso esses ids
    // entram por id.in. Se forem muitos, a lista e omitida e a consulta
    // volta a ser ampla: melhor trazer demais e filtrar em JS do que
    // montar uma URL gigante e quebrar a busca inteira.
    const ATIVOS = 'and(tipo.eq.problema,status.eq.concluido),tipo.eq.defeito_estoque';
    const TERMINAIS = 'tipo.eq.recuperado,tipo.eq.descartado';
    // b305 (revisao Codex #264) - a exclusao tambem pode estar marcada so
    // pelo status (ver /excluir), quando `tipo: 'defeito_excluido'` nao
    // passa no check da tabela. A consulta precisa achar os dois formatos,
    // senao a exclusao-por-status some da aba assim que a tela navega pra
    // ela.
    // b311 (revisao Codex #269, P1) - falta o TERCEIRO formato: quando nem
    // `tipo` nem `status` aceitam gravar (ver b310, fallback em /excluir), a
    // unica marca e o `estado_atual`. Sem entrar aqui NO OR, essas linhas
    // ficam de fora de `cond` e a aba Excluidos nunca as acha - mesmo que o
    // filtro de baixo (linha ~530) tente empurra-las pra ca, ele e um AND
    // com este `cond`, e um `problema/concluido` comum nao bate em nenhuma
    // das duas condicoes acima.
    const EXCLUIDOS = "tipo.eq.defeito_excluido,and(tipo.eq.problema,status.eq.cancelled),estado_atual.ilike.%REGISTRO EXCLUIDO%";
    const TUDO = ATIVOS + ',' + TERMINAIS + ',' + EXCLUIDOS;
    const MAX_IDS_NA_URL = 150;

    // seg4.3 (Codex): na aba "Com Defeito" nao basta pedir os ATIVOS — as
    // pecas ja recuperadas/descartadas POR PEDIDO (historico antigo, cujo
    // tipo da linha nunca foi marcado) continuam casando com ATIVOS. Elas
    // entravam no resultado, ocupavam vaga dentro do .limit(300) e so eram
    // removidas no pos-filtro em JS — empurrando defeito ativo ANTIGO pra
    // fora do limite, que e exatamente o sumico que este PR quer evitar.
    // Agora esses ids saem na propria consulta.
    function idsForaDoEstado(estado, porPedido) {
      if (estado !== 'defeito') return [];
      const ids = Object.keys(porPedido || {});
      return ids.length && ids.length <= MAX_IDS_NA_URL ? ids : [];
    }

    // seg4.4 (Codex) - quando os ids terminais passam do limiar da URL, a
    // exclusao nao entra na consulta e eles voltam a ocupar vaga dentro do
    // limite, empurrando defeito ativo antigo pra fora — o mesmo sumico,
    // so que a partir de 150 pecas resolvidas em vez de 300.
    //
    // Nesse caso o limite CRESCE pelo tanto que sera descartado depois, pra
    // que sobrem 300 linhas uteis. Teto de 1000 pra nao pedir a tabela
    // inteira.
    function limiteDaConsulta(estado, porPedido) {
      const base = 300;
      if (estado !== 'defeito') return base;
      const ids = Object.keys(porPedido || {}).length;
      if (!ids || ids <= MAX_IDS_NA_URL) return base;   // a exclusao entra na consulta
      return Math.min(base + ids, 1000);
    }

    function condicoesDoEstado(estado, porPedido) {
      const idsDe = (situacao) => Object.keys(porPedido || {})
        .filter((id) => porPedido[id] === situacao);

      if (estado === 'excluido') return EXCLUIDOS;

      if (estado === 'recuperado' || estado === 'descartado') {
        const ids = idsDe(estado);
        const base = 'tipo.eq.' + estado;
        if (!ids.length) return base;
        if (ids.length > MAX_IDS_NA_URL) return TUDO;   // volta ao amplo
        return base + ',id.in.(' + ids.join(',') + ')';
      }

      if (estado === 'defeito') return ATIVOS;   // os terminais saem por idsFora()

      return TUDO;   // 'todos'
    }

    async function buscar(termo, estado, porPedido) {
      const cond = condicoesDoEstado(estado, porPedido);
      const fora = idsForaDoEstado(estado, porPedido);
      let sel = dbc.from(T_DEV)
        // seg3 - A COLUNA DE DATA DA GOOD CHAMA created_at, NAO criado_em.
        // Este modulo foi portado da AMB, onde a tabela devolucoes_amb usa
        // criado_em. Aqui a tabela e a `devolucoes` ANTIGA da GOOD, que usa
        // created_at (server.js:2094, 2142, 2352 ja usam assim). Resultado:
        // a busca do Estoque de Defeitos morria com
        // "column devolucoes.criado_em does not exist" e a tela dizia
        // apenas "nada encontrado" — parecia produto inexistente.
        // A saida da API segue chamando o campo de `criado_em`, que e o que
        // o painel le (public/js/*.js): muda a ORIGEM, nao o contrato.
        // b311 (revisao Codex #269, P1) - `estado_atual` entra no SELECT: sem
        // ele, situacaoDe() (mais abaixo) nunca ve a marca de exclusao-so-
        // por-estado_atual (b310), porque o campo simplesmente nao chega no
        // JS - o filtro de banco (linha ~530) ate acerta quem sai da lista,
        // mas a classificacao em JS ficava cega pro mesmo dado.
        .select('id, produto_sku, produto_titulo, localizacao, defeito_qtd, produto_qtd, problema_descricao, problema_fotos, tipo, status, estado_atual, funcionario, nf_numero, created_at')
        ...
        // seg4 - NA GOOD, "problema" e TIPO, nao STATUS.
        // Este filtro tambem veio da AMB, onde a triagem grava
        // status:'problema' (compat-AMB.js:1109). Na GOOD o registro nasce
        // com tipo:'problema' e status:'pendente' (server.js:2403-2404), e
        // todo o resto do server ja filtra assim -- .in('tipo', ['problema',
        // 'defeito_estoque']) nas linhas 2827 e 4238.
        //
        // Efeito: depois de consertar a coluna de data (seg3), a busca
        // rodava mas voltava SEMPRE vazia -- inclusive com 9 problemas
        // reportados visiveis na tela do painel.
        //
        // A REGRA DE NEGOCIO (v3.98, server.js:4249-4256): defeito vindo de
        // DEVOLUCAO so existe de verdade aqui DEPOIS da NF emitida e
        // concluida — antes disso a peca ainda nao foi pro deposito
        // DEFEITO, foi pro GERAL, que volta pra venda. Por isso
        // tipo='problema' entra so com status='concluido': liberar as
        // pendentes deixaria retirar peca, recuperar ou descartar antes
        // de o fluxo fiscal terminar (apontamento P1 do Codex).
        //
        // recuperado/descartado entram porque, ao AUTORIZAR o pedido,
        // este modulo troca o tipo da linha (linhas ~1037-1046). Sem eles
        // a peca sumiria das abas Recuperados/Descartados no refresh
        // seguinte, em vez de ficar no historico.
        //
        // status.eq.problema (molde da AMB) NAO entra: la a triagem grava
        // status='problema' sem passar por esta regra de conclusao, e
        // aceitar isso na GOOD abriria excecao justamente na trava fiscal.
        .or(cond);   // v4.62 - registro excluido some da lista
      // AND com o .or() acima: tira os ja resolvidos por pedido ANTES do limite
      if (fora.length) sel = sel.not('id', 'in', '(' + fora.join(',') + ')');

      // b310/b311 - ⚠️ QUEM TEM MARCA DE EXCLUIDO SAI DAS LISTAS ATIVAS E
      // TERMINAIS (senao um `problema/concluido` marcado so no estado_atual
      // continua batendo em ATIVOS normalmente e aparece como se nada
      // tivesse acontecido).
      //
      // A exclusao nao consegue mudar `tipo` nem `status`: as duas colunas
      // tem lista fechada e nenhum valor de exclusao passa no check —
      // tentei tres, e cada um custou uma rodada do dono. Entao ela escreve
      // a marca no `estado_atual`, que e TEXTO LIVRE, e o filtro mora aqui.
      //
      // ⚠️ revisao Codex #269 (P1): `.not('estado_atual','ilike',...)`
      // SOZINHO some com quem tem estado_atual NULL — em SQL, `NOT (NULL
      // ILIKE x)` da NULL, e PostgREST trata NULL como falso. Quase toda
      // linha ATIVA nao tem marca nenhuma (nunca foi excluida), entao esse
      // filtro sozinho esvaziava as proprias abas ativas. O OR com is.null
      // cobre quem nunca teve a coluna tocada.
      //
      // ⚠️ e revisao Codex #269 (P2): SO nas abas ativas/terminais de
      // verdade. Na aba Excluidos a marca ja entra pelo `cond` (EXCLUIDOS,
      // acima) — repetir o filtro aqui excluiria excluidos ANTIGOS que
      // nunca tiveram estado_atual escrito (so tipo='defeito_excluido').
      // E em "todos" o pedido e mesmo trazer TUDO, exclusao inclusa.
      if (estado !== 'excluido' && estado !== 'todos') {
        sel = sel.or('estado_atual.is.null,estado_atual.not.ilike.%REGISTRO EXCLUIDO%');
      }
      sel = sel
        .order('created_at', { ascending: false })
        .limit(limiteDaConsulta(estado, porPedido));
      const r = await sel;
    ...
    try {
      // seg4.2 - isto vinha DEPOIS da busca. Precisa vir antes: e o que
      // permite a condicao da aba entrar na consulta, e nao so no JS
      // depois do limite.
      const porPedido = {};
      try {
      ...
      // b312 (revisao Codex #269, P2) - os tres formatos de exclusao (tipo,
      // status ou so a marca no estado_atual - ver b310/b311 acima) moram
      // agora em marcadoExcluido() (lib/defeito-excluido.js), a MESMA
      // funcao que o /excluir, o /estado e a ficha usam. Antes cada lugar
      // testava a marca com seu proprio regex - este aqui era
      // case-sensitive enquanto o filtro de banco (linha ~570) usa ILIKE;
      // uma marca gravada com case diferente do usual passava pelo SQL mas
      // continuava classificada como ativa aqui, e sumia so da aba
      // Excluidos.
      const situacaoDe = (x) => {
        if (marcadoExcluido(x)) return 'excluido';
        if (x.tipo === 'recuperado' || x.tipo === 'descartado') return x.tipo;
        return porPedido[x.id] || 'defeito';
      };

      // revisao Codex #270 (P2) - a contagem por aba vinha de buscar(), que
      // tem .limit(300/1000) pra tela de itens. Sem termo de busca, `cond`
      // (+ o filtro de estado_atual acima) JA seleciona exatamente as linhas
      // da aba - a mesma regra que situacaoDe() reproduz em JS -, entao um
      // count exato no banco (sem limit) da o numero certo mesmo com mais
      // de 300 registros. So cai pro caminho antigo (busca + filtro em JS)
      // quando ha termo, porque a combinacao com SKU/localizacao/titulo so
      // e resolvida em JS.
      //
      // ⚠️ revisao Codex #270, rodada 2 - o mesmo count exato quebra quando
      // porPedido passa de MAX_IDS_NA_URL: condicoesDoEstado() alarga
      // 'recuperado'/'descartado' pra TUDO (comentario "volta ao amplo" ali
      // em cima) pra nao montar um id.in(...) gigante, e idsForaDoEstado()
      // some com a exclusao de 'defeito' pelo mesmo motivo. Um head-count
      // direto nessa condicao alargada conta a TABELA INTEIRA (ou o defeito
      // ja resolvido por pedido) como se fosse so a aba - nao um numero
      // truncado como antes, um numero ERRADO pra mais. buscar()+filtro
      // (o caminho antigo) nao tem esse problema: o filtro em JS por
      // situacaoDe() sempre restringe a aba certa, mesmo quando a consulta
      // trouxe mais que o necessario - so fica sujeito ao limiteDaConsulta
      // de novo, que e a mesma limitacao que ja existia antes deste PR.
      function alargouCondicaoDaAba(estado, porPedido) {
        const ids = Object.keys(porPedido || {});
        if (estado === 'recuperado' || estado === 'descartado') {
          return ids.filter((id) => porPedido[id] === estado).length > MAX_IDS_NA_URL;
        }
        if (estado === 'defeito') return ids.length > MAX_IDS_NA_URL;
        return false;
      }
      async function contarAba(termo, estado, porPedido) {
        if (termo || alargouCondicaoDaAba(estado, porPedido)) {
          const linhas = await buscar(termo, estado, porPedido);
          return linhas.filter(x => situacaoDe(x) === estado).length;
        }
        const cond = condicoesDoEstado(estado, porPedido);
        const fora = idsForaDoEstado(estado, porPedido);
        let sel = dbc.from(T_DEV).select('id', { count: 'exact', head: true }).or(cond);
        if (fora.length) sel = sel.not('id', 'in', '(' + fora.join(',') + ')');
        if (estado !== 'excluido' && estado !== 'todos') {
          sel = sel.or('estado_atual.is.null,estado_atual.not.ilike.%REGISTRO EXCLUIDO%');
        }
      ...
      let linhas = await buscar(q, estado, porPedido);
      let viaEan = null;
      ...
      // ═══════════════════════════════════════════════════════════════
      // o pos-filtro continua: ele resolve o caso "linha ativa que TEM
      // pedido autorizado" (historico antigo), que a condicao de banco
      // sozinha nao separa.
      if (estado !== 'todos') {
      ...
      if (!linhas.length && /^\d{12,14}$/.test(q) && typeof chamarBling === 'function') {
        try {
          ...
          if (prod && prod.codigo) {
            ...
            linhas = await buscar(String(prod.codigo).toLowerCase(), estado, porPedido);
      ...
      // seg4.4 (Codex) - conta ABA POR ABA (nao mais pelo total combinado)
      // b314 - a aba Excluidos tambem entra no laco agora: antes ela ficava
      // de fora e o botao sempre mostrava (0), mesmo com registros la —
      // "já excluí 2, e mostra 0" (relato de 13/09). A contagem em si vem de
      // contarAba() (acima), que evita o limite de linhas da tela.
      const contagem = { defeito: 0, recuperado: 0, descartado: 0, excluido: 0 };
      for (const aba of ['defeito', 'recuperado', 'descartado', 'excluido']) {
        contagem[aba] = await contarAba(termoContagem, aba, porPedido);
      }
      ...
      res.json({
        ...
        itens: linhas.map(x => ({
          ...
          quem: x.funcionario,
          criado_em: x.created_at,   // seg3 - origem created_at, nome de saida mantido
    ...
    try {
      ...
      res.json({
        ...
        item: {
          ...
          criado_em: item.created_at || item.criado_em,   // seg3 - aqui e select('*'): nao quebrava, so vinha vazio
          status: item.status,
          tipo: item.tipo,               // b126 - recuperado / descartado / defeito_estoque
          // b312 (revisao Codex #269, P1) - so olhava tipo==='defeito_excluido':
          // um registro excluido pelo fallback do estado_atual (b310) chegava
          // aqui como se estivesse ativo, escondendo o botao Restaurar (so-
          // admin) e liberando as acoes de retirar peca/pedido - a mesma
          // exclusao que a lista (situacaoDe) ja reconhecia.
          situacao: marcadoExcluido(item) ? 'excluido'
  ...
  app.put('/api/defeitos/:id/estado', requerLogin, async (req, res) => {
    ...
    let r;
    try {
      // b311 (revisao Codex #269, P1) - ⚠️ ESTA E A UNICA TRAVA DA EXCLUSAO
      // QUANDO A MARCA E O ESTADO_ATUAL (b310): sem o tipo/status mudados,
      // nada mais no banco impede um usuario logado comum de reescrever
      // (ou simplesmente APAGAR) este campo por aqui e reativar sozinho um
      // registro que so o admin pode excluir/restaurar. Antes desta trava,
      // limpar o texto aqui tinha o MESMO efeito pratico do botao
      // Restaurar, sem passar pela checagem de admin nem pelo /restaurar.
      //
      // b312 (revisao Codex #269, P2) - ⚠️ a trava virou um SELECT separado
      // do UPDATE: entre os dois, um admin podia gravar a marca de exclusao
      // e este UPDATE (que ja tinha passado no cheque) sobrescrevia ela sem
      // saber que tinha acabado de chegar - as duas chamadas terminavam OK,
      // mas a exclusao "desaparecia". Agora a MESMA condicao entra no WHERE
      // do UPDATE: e atomico, o banco so grava se a linha nao estiver
      // marcada NO MOMENTO da gravacao. O `is.null` cobre quem nunca teve a
      // coluna tocada (senao `NOT (NULL ILIKE x)` vira NULL/falso e a
      // condicao nunca bate - mesmo cuidado do filtro de lista, linha ~570).
      const up = await cli().from(T_DEV)
        .update({ estado_atual: texto || null }).eq('id', req.params.id)
        .or('estado_atual.is.null,estado_atual.not.ilike.%REGISTRO EXCLUIDO%')
        .select().limit(1);
      if (up.error) throw new Error(up.error.message);
      const registro = (up.data || [])[0] || null;
      if (!registro) {
        const existe = await cli().from(T_DEV).select('id').eq('id', req.params.id).limit(1);
        if (!(existe.data || []).length) {
          return res.status(404).json({ ok: false, erro: 'registro nao encontrado' });
        }
        return res.status(400).json({
          ok: false,
          erro: 'este registro esta excluido - peca a um admin pra RESTAURAR antes de editar o estado',
        });
      }
      r = { ok: true, registro };
    } catch (e) { return res.status(500).json({ ok: false, erro: String(e.message || e) }); }
  ...
  app.post('/api/defeitos/pedido/:id/decidir', requerLogin, async (req, res) => {
    ...
    try {
      ...
      if (pedido && pedido.defeito_id && acao === 'autorizar') {
        ...
        try {
          // b305 - ⚠️ `recuperado`/`descartado` TAMBEM nao passam no check.
          //
          // Mesma familia do `defeito_excluido`: o codigo LE esses tipos, a
          // tabela nao aceita GRAVAR. Aqui o efeito seria pior — o admin
          // autoriza a peca, o update falha calado (o catch abaixo engole), e
          // a peca fica ATIVA pra sempre: o galpao ve como pendente algo que
          // ja foi decidido.
          //
          // Entao tento o tipo e, se o check recusar, marco pelo `status`.
          //
          // b305 (revisao Codex #264) - ⚠️ NAO pode ser `status: 'concluido'`:
          // a peca ativa ja ESTA com status='concluido' (e o par que
          // /api/defeitos e /api/defeitos/por-sku tratam como disponivel
          // pra canibalizacao — server.js:3774/5514). Gravar o mesmo valor
          // de novo e NO-OP: a peca autorizada continuava aparecendo pro
          // estoquista como se ainda estivesse la, liberada pra retirar
          // peca de novo. Uso 'delivered' (sentinela livre da lista fechada,
          // sem outro uso na tabela `devolucoes`) pra sair de fato do par
          // que essas rotas conferem. A aba certa (recuperado/descartado)
          // continua vindo do pedido (porPedido, abaixo), que nao depende
          // do tipo/status desta linha — so precisa deixar de contar como
          // ATIVA/liberada.
          let rDec = await cli().from(T_DEV).update({
          ...
          if (rDec.error && /check constraint/.test(String(rDec.error.message || ''))) {
            await cli().from(T_DEV).update({
              status: 'delivered',
              estado_atual: recuperada
                ? 'RECUPERADA - liberada por ' + req.usuario + ' em ' + quando
                : 'DESCARTE AUTORIZADO por ' + req.usuario + ' em ' + quando,
            }).eq('id', pedido.defeito_id);
            console.log('[DEFEITOS] decisao gravada via status (o tipo nao passa no check)');
          }
