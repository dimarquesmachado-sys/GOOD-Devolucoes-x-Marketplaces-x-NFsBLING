...
module.exports = function registrarCicloDefeitos(app, deps) {
  ...
  app.post('/api/defeitos/:id/restaurar', requerLogin, async (req, res) => {
    ...
    try {
      ...
      // b305 - ⚠️ 3o FALLBACK: o TIPO pode nao passar no check da tabela.
      //
      // CASO REAL (13/09): o dono clicou "Excluir este registro" e levou
      //   new row violates check constraint "devolucoes_tipo_check"
      //
      // A coluna `tipo` tem lista FECHADA, e `defeito_excluido` nao esta
      // nela — igual `defeito_estoque`, que ja tinha me pegado. O codigo LE
      // esses valores em varios lugares, mas a tabela nunca aceitou GRAVAR.
      //
      // ⚠️ O fallback que existia tirava `tipo_anterior` (coluna que pode
      // faltar) — nao resolve quando o problema e o VALOR do tipo.
      //
      // Aqui marco pelo `status`. Conferi que funciona: os ATIVOS da caixa
      // sao `and(tipo.eq.problema,status.eq.concluido)` — mudando o status,
      // o registro SAI das listas ativas do mesmo jeito, e o `estado_atual`
      // conta a historia pra quem abrir.
      //
  ...
  app.get('/api/defeitos/lista', requerLogin, async (req, res) => {
    ...
    const TUDO = ATIVOS + ',' + TERMINAIS + ',tipo.eq.defeito_excluido';
    ...
    function condicoesDoEstado(estado, porPedido) {
      ...
      if (estado === 'excluido') return 'tipo.eq.defeito_excluido';
    ...
    try {
      ...
      if (pedido && pedido.defeito_id && acao === 'autorizar') {
        ...
        try {
          ...
          // (`concluido`) — que sai dos ATIVOS pelo mesmo caminho da exclusao.
