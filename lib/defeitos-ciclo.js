...
module.exports = function registrarCicloDefeitos(app, deps) {
  ...
  app.post('/api/defeitos/:id/restaurar', requerLogin, async (req, res) => {
    ...
    try {
      ...
      const excluidoPorMarca = !excluidoPorTipo && !excluidoPorStatus
        && /REGISTRO EXCLUIDO/.test(String(item.estado_atual || ''));
  ...
  app.get('/api/defeitos/lista', requerLogin, async (req, res) => {
    ...
    try {
      ...
      const situacaoDe = (x) => {
        ...
        // b305 (revisao Codex #264) - exclusao-por-status (ver /excluir):
        // o tipo ficou 'problema' porque 'defeito_excluido' nao passou no
        // check, mas o status virou 'cancelled' pra marcar mesmo assim.
        if (x.tipo === 'problema' && x.status === 'cancelled') return 'excluido';
        // ⚠️ b307/b311 (revisao Codex #269, P1) - e o marcador que a tabela
        // sempre ACEITA: o `estado_atual`, TEXTO LIVRE.
        //
        // `cancelled` aparece no codigo mas a TABELA nao aceita pra
        // `status` (o dono levou `violates check constraint
        // "devolucoes_status_check"`); e o b310 provou que ATE o `tipo`
        // pode ser recusado (`defeito_excluido` nao passa no
        // `devolucoes_tipo_check`). Quando os DOIS sao recusados, o
        // fallback em /excluir grava SO a marca, e tipo/status ficam
        // EXATAMENTE como estavam antes (ex.: problema/concluido, o par
        // que marca peca ATIVA) — por isso o teste nao pode depender de um
        // status especifico como o b307 fazia (so pendente): a marca vale
        // sozinha, em qualquer tipo/status.
        if (/REGISTRO EXCLUIDO/.test(String(x.estado_atual || ''))) return 'excluido';
  ...
  app.put('/api/defeitos/:id/estado', requerLogin, async (req, res) => {
    ...
    try {
      ...
      const atual = await cli().from(T_DEV).select('estado_atual').eq('id', req.params.id).limit(1);
      const marcado = /REGISTRO EXCLUIDO/.test(String((atual.data || [])[0]?.estado_atual || ''));
      if (marcado) {
        return res.status(400).json({
          ok: false,
