...
module.exports = function registrarCicloDefeitos(app, deps) {
  ...
  app.post('/api/defeitos/:id/restaurar', requerLogin, async (req, res) => {
    ...
    try {
      const rI = await dbc.from(T_DEV).select('id, tipo, tipo_anterior, status, shipment_id, order_id, pack_id, buyer_nome, pedido_bling_numero, nf_numero, nf_chave, nf_id_bling').eq('id', req.params.id).limit(1).then(r => r).catch(() => ({ data: null, error: { message: 'erro' } }));
      let item = (rI.data || [])[0] || null;
      if (!item && rI.error) {
        const r2 = await dbc.from(T_DEV).select('id, tipo, status, shipment_id, order_id, pack_id, buyer_nome, pedido_bling_numero, nf_numero, nf_chave, nf_id_bling').eq('id', req.params.id).limit(1);
        item = (r2.data || [])[0] || null;
      }
      ...
      // Sem a coluna tipo_anterior, o fallback precisa adivinhar de onde a
      // linha veio. O criterio antigo ('status === problema') e o molde da
      // AMB; na GOOD o status e 'pendente'/'concluido', entao TODA linha de
      // devolucao voltaria como defeito_estoque e sairia do historico fiscal
      // (apontamento do Codex).
      //
      // O sinal que separa os dois na GOOD: defeito lancado do ESTOQUE e
      // controle interno, sem venda por tras (server.js:4198-4203).
      //
      // seg4.2 (Codex): NF nao serve sozinha como sinal — devolucao SEM nota
      // existe e e suportada (a triagem aceita shipment_id OU nf_chave OU
      // protocolo Magalu, server.js:2122, e concluir nao exige NF). Uma
      // devolucao de Magalu sem nota voltaria como defeito_estoque e sairia
      // do historico fiscal calada.
      //
      // O identificador que TODA devolucao tem e o shipment_id: ele e
      // gravado em cascata (shipment > chave da NF > protocolo Magalu,
      // server.js:2384) e nunca existe num defeito de estoque.
      //
      // revisao do Codex no #249: desde que o defeito de estoque
      // passou a gravar um shipment_id PROPRIO (`DEF-<timestamp>-<aleatorio>`,
      // server.js:5165) pra satisfazer a coluna NOT NULL, esse shipment_id
      // sintetico nao pode contar como prova de venda — senao excluir e
      // restaurar um defeito de estoque o devolve como 'problema' e ele
      // some do estoque de defeitos (fica 'registrado', nao 'concluido').
      const shipmentReal = item.shipment_id && !/^DEF-/.test(String(item.shipment_id));
      const veioDeDevolucao = !!(shipmentReal || item.order_id || item.pack_id
        || item.buyer_nome || item.pedido_bling_numero
        || item.nf_numero || item.nf_chave || item.nf_id_bling);
      const tipoVolta = item.tipo_anterior !== undefined && item.tipo_anterior !== null
        ? item.tipo_anterior
        : (String(item.status || '') === 'problema' ? null
           : (veioDeDevolucao ? 'problema' : 'defeito_estoque'));
