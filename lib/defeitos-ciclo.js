...
module.exports = function registrarCicloDefeitos(app, deps) {
  ...
  async function entradaNoEstoque({ sku, quantidade, observacao }) {
    ...
    // deposito GERAL da GOOD - a peca esta boa, volta a ser vendavel
    const deposito = DEPOSITO_GERAL || process.env.GOOD_DEPOSITO_GERAL || '';
    if (!deposito) return { ok: false, erro: 'GOOD_DEPOSITO_GERAL nao configurado no Render' };
    try {
      const rP = await chamarBling('/produtos?codigo=' + encodeURIComponent(sku) + '&limite=3');
      ...
      const r = await chamarBling('/estoques', {
        method: 'POST',
        body: JSON.stringify({
          produto: { id: prod.id },
  ...
  app.get('/api/defeitos/lista', requerLogin, async (req, res) => {
    ...
    try {
      ...
      if (!linhas.length && /^\d{12,14}$/.test(q) && typeof chamarBling === 'function') {
        try {
          const rB = await chamarBling(`/produtos?gtin=${encodeURIComponent(q)}&limite=3`);
