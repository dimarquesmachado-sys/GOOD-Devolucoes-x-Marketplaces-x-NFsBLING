...
function montarEmpresas({ app, empresas, criarApp, ambiente = process.env, logger = console }) {
  ...
  for (const empresa of lista) {
    ...
    try {
      ...
      // ⚠️ A ROTA RESPONDE 503 E EXPLICA, em vez de dar 404.
      //
      // Sem isto a rota simplesmente não existe, e 404 manda o dono procurar
      // no lugar errado: parece que a empresa nunca foi configurada, quando
      // na verdade ela falhou por um motivo que está no log.
      app.use(empresa.rota, (req, res) => res.status(503).json({
        ok: false,
...
module.exports = { montarEmpresas, diagnostico, freada, nomeFreio };
