...
function traduzir(d, empresa) {
  ...
  return {
    ...
    rastreio: txt(d.tracking, 120),
    nf_numero: txt(d.nf, 40),
    nf_chave: txt(d.chave_nota, 60),

    cliente_nome: txt(d.cliente, 200),
    produto_sku: txt(d.sku, 120),
    produto_titulo: txt(d.produto, 400),
    produto_qtd: d.qtd != null ? parseInt(d.qtd, 10) || null : null,
    ...
    criado_no_mkt: (d.dias_em_transito != null && Number.isFinite(Number(d.dias_em_transito)))
      ? new Date(Date.now() - Number(d.dias_em_transito) * 864e5).toISOString()
      : null,
    atualizado_no_mkt: null,
