...
function normalizarCaminhoPedido(caminhoBruto) {
  ...
  let limpo = atual.replace(/\\/g, '/').replace(/\/{2,}/g, '/');

  // seg2.2 (2o apontamento do Codex no PR #85): resolver `.` e `..` ANTES
  // de comparar. `/./painel-devolucoes.html` e `/admin/../painel-devolucoes.html`
  // chegam ao static, que resolve os segmentos e acha o arquivo — mas nao
  // casavam com a comparacao literal aqui. path.posix.normalize faz isso.
  if (limpo.indexOf('.') !== -1) {
    const semBarraFinal = limpo.length > 1 && limpo.endsWith('/');
    limpo = path.posix.normalize(limpo);
    if (semBarraFinal && !limpo.endsWith('/')) limpo += '/';
  }
  if (!limpo.startsWith('/')) limpo = '/' + limpo;   // normalize pode comer a raiz
