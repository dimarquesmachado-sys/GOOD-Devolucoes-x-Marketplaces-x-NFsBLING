...
function configDaEmpresa(empresa) {
  ...
  return {
    ...
    // ⚠️ b368.1 (Codex, P2) - `PREFIXO` SOZINHO NAO SERVE PRA MONTAR ROTA.
    //
    // `ficha.prefixoRota` da GOOD e '' de proposito (linha 70 acima), mas o
    // bootstrap monta a GOOD em `/good` (mesmo fallback do `caminhoMontado`
    // usado no cookie e no redirectUri, linha 96). Quem usar `PREFIXO` puro
    // pra montar link cai na raiz em vez de `/good`.
    //
    // 📌 Exponho o caminho JA COM o fallback aplicado, pronto pra concatenar.
    PREFIXO_ROTA: caminhoMontado,
