// ============================================================
// lib/nf-pessoa.js — COMUM as duas empresas (b240, 04/09)
//
// Era copia: `lib/nf-pessoa.js` na GOOD e `lib-AMB/nf-pessoa-AMB.js` na
// AMB. Medi antes de unificar e o resultado surpreendeu — 72% iguais, mas
// a divergencia NAO era so as 2 funcoes extras da AMB:
//
//   nomesBatem, acharNfDevolucaoBling   so na AMB (b255, b316)
//   TETO_PAGINAS + `_truncado`          so na AMB (b261)
//   diasIncompletos                     so na AMB (b261)
//   "pagina enquanto vier CHEIA"        so na AMB (b239)
//
// Os tres ultimos sao CONSERTOS DE BUG que a GOOD nunca recebeu: o teto de
// 4 paginas parava mesmo com a 4a cheia, entao num dia movimentado a NF
// procurada ficava na 5a e a rota dizia "nao existe". A GOOD esta assim em
// producao hoje.
//
// Adotei a versao da AMB como comum: unifica E conserta a GOOD de uma vez.
// (regra do dono: "tem que ter para AMBOS" — aqui era a GOOD que tinha
// ficado pra tras.)
//
// v3.43 original:
// ------------------------------------------------------------
...
module.exports = function criarNfPessoa({ chamarBling, sleep }) {
  ...
  async function acharNfDevolucaoBling({ cliente, sku, desde, ate, naturezaId } = {}) {
    ...
    // ⚠️ b402 - O PADRAO DA AMB SAIU. Este arquivo e de `/lib`, compartilhado.
  //
  // Era `naturezaId || process.env.AMB_NATUREZA_DEVOLUCAO || '15110882041'`
  // — o id da AMBTotal CRAVADO, num modulo que nao e dela. E os 2 chamadores
  // nao passavam `naturezaId`, entao TODOS caiam nele.
  //
  // ⚠️ A Girassol buscaria a NF de devolucao pela natureza da AMBTotal: nao
  // acharia as dela, e poderia achar as DA AMB.
  //
  // 📌 Agora quem chama INFORMA. Sem natureza, a busca recusa com motivo
  // claro — em vez de procurar na natureza errada e dizer "nao achei".
  //
  // ⚠️ b402 r2 (review do Codex) - `naturezasDevolucaoIds()` pode devolver
  // uma LISTA separada por virgula (e' o formato dela em lib/empresas.js,
  // ex. GOOD '5776118802,15110882187'). Comparar a lista inteira como uma
  // string unica contra o id de cada NF nunca batia — nenhuma nota passava.
  // Mesmo split usado em server.js:5317 e app-AMB.js:2032/2491.
  const naturezas = String(naturezaId || '').split(',').map((x) => x.trim()).filter(Boolean);
  if (!naturezas.length) {
    return { ok: false, motivo: 'sem a natureza de devolucao da empresa — '
      + 'quem chama precisa informar `naturezaId` (vem da ficha, em '
      + '`fiscal.naturezasDevolucaoIds()`)' };
  }
    ...
    for (let pg = 1; pg <= 4; pg++) {
      ...
      for (const nf of lista) {
        const natId = String(nf.naturezaOperacao?.id || nf.naturezaOperacao || '');
        const natOk = natId && naturezas.indexOf(natId) >= 0;
        if (!natOk) continue;
