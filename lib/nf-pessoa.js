...
module.exports = function criarNfPessoa({ chamarBling, sleep }) {
  ...
  async function buscarNFsPorNumero(numero, serieExplicita = null, opts = {}) {
    ...
    // ---- 1) FILTRO DIRETO (barato: 1 chamada, caso o Bling honre ?numero=) ----
    try {
      await sleep(200);
      ...
      log('filtro-numero', { ok: r.ok, status: r.status || null, qtd: lista.length, hits: hits.length });
      if (hits.length > 0) {
        console.log(`[buscarNFsPorNumero] ${alvoStr}: ${hits.length} NF(s) via filtro ?numero= (1 chamada)`);
        return hits.map(nf => ({ id: String(nf.id), serie: serOf(nf), numero: String(nf.numero) }));
      }
    } catch (e) { log('filtro-numero', { ERRO: e.message || String(e) }); }
      ...
      if (diaAlvo != null) {
        ...
        for (const dd of [0, 1, -1, 2, -2]) {
          ...
          for (let pg = 1; pg <= 4; pg++) {
            ...
            if (m) { achou = { id: String(m.id), serie, numero: String(m.numero) }; break; }
