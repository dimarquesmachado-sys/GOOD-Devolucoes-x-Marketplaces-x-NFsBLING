...
function separar(corpo, agora) {
  ...
  // b183: separo as origens, porque `aguardando_analise` tem significado
  // proprio — e a lista de quem ELES ja identificaram como no relogio.
  const daLista = (corpo && corpo.aguardando_analise) || [];
  const outros = (corpo && (corpo.devolucoes || corpo.registros)) || [];
  const registros = outros.length ? outros : daLista;
  const veioDaListaDeles = !outros.length && daLista.length > 0;
  ...
  for (const r of registros) {
    ...
    if (av.perdido_por_revelia) { perdidas.push(item); continue; }
    if (av.risco === 'atencao' || av.risco === 'urgente') { emRisco.push(item); continue; }

    // b183 (Codex): a lista `aguardando_analise` JA e a selecao deles de
    // quem esta no relogio. Se o registro veio de la e eu nao consegui
    // avaliar (sem eventos crus, que e o formato real), descartar seria
    // jogar fora exatamente quem o alerta existe pra mostrar.
    //
    // Entao: veio da lista deles + sem desfecho meu = entra, marcado como
    // vindo pronto. Melhor mostrar sem os dias calculados do que nao
    // mostrar.
    if (veioDaListaDeles && !av.fechado && av.risco === null) {
      emRisco.push({ ...item, risco: 'atencao', origem: 'lista_do_servico',
        nota: 'sinalizado pelo proprio TikTok como aguardando analise; sem eventos pra calcular os dias' });
    }
  }
