...
function marketplaceDaNF(nf) {
  // b210.1 (Codex): `loja` no Bling e OBJETO com id — `normalizar` viraria
  // "objectobject" e casaria com nada. Pego o texto de onde ele existe.
  const cand = nf ? [
    typeof nf.loja === 'string' ? nf.loja : (nf.loja && (nf.loja.nome || nf.loja.descricao)),
    nf.nomeLoja,
    nf.origemLojaVirtual,
    nf.lojaVirtual && nf.lojaVirtual.nome,
    nf.loja && nf.loja.id != null ? 'loja-' + nf.loja.id : null,   // ultimo recurso
  ] : [];
  const bruto = normalizar(cand.find((x) => typeof x === 'string' && x.trim()) || '');
...
// ── b210: o mapa se APRENDE, em vez de eu ficar pedindo os numeros ──
//
// [stated] "vendas normais da matriz tudo série 1. daí cada marketplace com
// operação fullfilment vai ter 1 série específica, pois são NFs q o próprio
// programa faturador do marketplace emite."
//
// A regra e firme, e toda nota do Bling traz SERIE e ORIGEM juntas. Entao
// nao preciso que ele lembre dos numeros: cada nota que passa por aqui
// ensina uma linha do mapa.
//
// Conservador de proposito: so registra o que VIU, e a rota de diagnostico
// mostra o aprendido pra ele conferir. Se duas origens aparecerem na mesma
// serie, guardo as duas e marco como duvidoso — nao escolho sozinho.
const APRENDIDO = new Map();   // 'empresa|serie' -> { origens: Map(origem->contagem) }

function aprender(empresa, serie, nf) {
  const n = parseInt(String(serie || '').replace(/^0+/, ''), 10);
  const origem = marketplaceDaNF(nf);
  if (!Number.isFinite(n) || !origem || n === 1) return;   // serie 1 e a matriz
  const k = String(empresa || '').toLowerCase() + '|' + n;
  if (!APRENDIDO.has(k)) APRENDIDO.set(k, new Map());
  const m = APRENDIDO.get(k);
  m.set(origem, (m.get(origem) || 0) + 1);
}

/** O que o mapa aprendeu, pra ele conferir na rota de diagnostico. */
function mapaAprendido() {
  const out = [];
  for (const [k, origens] of APRENDIDO) {
    const [empresa, serie] = k.split('|');
    const lista = [...origens.entries()].sort((a, b) => b[1] - a[1]);
    out.push({
      empresa,
      serie: String(serie).padStart(3, '0'),
      origem: lista[0][0],
      vistas: lista[0][1],
      duvidoso: lista.length > 1,
      outras: lista.length > 1 ? lista.slice(1).map(([o, c]) => o + '(' + c + ')') : undefined,
    });
  }
  return out.sort((a, b) => (a.empresa + a.serie).localeCompare(b.empresa + b.serie));
}
...
function canalDaSerie(serie, empresa) {
  ...
  if (n === 1) return 'propria';
  // b210: o que o proprio sistema aprendeu observando as notas
  const ap = APRENDIDO.get(String(empresa || '').toLowerCase() + '|' + n);
  if (ap && ap.size === 1) return [...ap.keys()][0] + '-full';
...
module.exports = { serieDaChave, marketplaceDaNF, avaliar, escolher, normalizar,
  canalDaSerie, ehDoFull, SERIE_CANAL, aprender, mapaAprendido, _APRENDIDO: APRENDIDO };
