...
function registrarPreventiva(opcoes) {
  ...
  async function preventiva({ forcar = false } = {}) {
    ...
    // b273 (review do Codex) - O LOCK SEGUE A OPERAÇÃO REAL, não o prazo de
    // resposta. Antes eu cortava com `Promise.race` e liberava `emVoo` —
    // mas a renovação original continuava viva, ainda usando o refresh de
    // uso único; um batimento ou 401 logo depois começaria outra rotação
    // sobre o mesmo refresh. Agora: o lock só cai quando a operação de fato
    // termina; o prazo serve apenas pra RESPONDER sem pendurar a requisição.
    ...
    estado.emVoo = operacao;
    operacao.then(() => { estado.emVoo = null; }, () => { estado.emVoo = null; });

    return Promise.race([
      operacao,
      new Promise((ok) => setTimeout(() => ok({
        ok: false, empresa, integracao, ainda_rodando: true,
        erro: 'a renovacao passou de 45s e segue em andamento — o lock continua preso ate ela terminar',
      }), 45000)),
    ]);
  }
  ...
  function ligar({ atrasoMs = 2 * 60 * 1000 } = {}) {
    ...
    // b274 (review do Codex) - o timer chama `estado.preventiva`, não a função
    // léxica deste registro. Se uma fábrica registrar de novo o mesmo
    // empresa/integração depois do agendamento, o timer antigo continuaria
    // renovando pela closure ANTIGA (com o `renovar`/`persistiu` da primeira
    // instância, possivelmente já obsoletos). Com a indireção, o timer
    // existente passa a usar automaticamente o registro mais recente.
    const disparar = () => { try { estado.preventiva().catch(() => {}); } catch (e) { /* nao trava */ } };
    const primeiro = setTimeout(() => {
      ...
      disparar();
      const bat = setInterval(disparar, UMA_HORA);
...
async function relatorio({ forcar = false, prazoMs = 25000 } = {}) {
  ...
  // b274 (review do Codex) - SEMPRE em série. Eu serializava só o `?forcar=1`,
  // mas uma chamada normal também dispara rotações DE VERDADE quando algum
  // carimbo está vencido ou ausente — e aí valia o mesmo risco: várias
  // escritas de env var na fila, e se a primeira disparar redeploy o processo
  // pode morrer antes das outras gravarem, perdendo refresh de uso único.
  // Quando todas pulam (o caso comum), a série é instantânea de qualquer jeito.
  const medir = async (e) => {
    try {
  ...
  // b273 (review do Codex) - com `?forcar=1` são rotações DE VERDADE, e cada
  // uma grava env var no Render. Disparar as seis juntas colocaria seis
  // escritas na fila; se a primeira disparar redeploy, o processo pode morrer
  // antes das demais gravarem — e aí seriam refresh de uso único trocados no
  // marketplace e PERDIDOS aqui. Forçado roda em série (cada uma grava antes
  // da próxima começar); sem forçar, quase tudo é "pulou" e vai em paralelo.
  const resultados = [];
  for (const e of itens) {
    if (Date.now() >= limite) { resultados.push([e, { ok: false, erro: 'nao deu tempo nesta chamada — rode de novo' }]); continue; }
    resultados.push(await medir(e));
  }
  const out = {};
