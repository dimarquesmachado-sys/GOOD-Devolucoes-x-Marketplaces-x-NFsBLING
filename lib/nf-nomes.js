...
module.exports = ({ chamarBling }) => {
  ...
  async function construirIndice(opts = {}) {
    ...
    for (let pg = 1; pg <= maxPaginas; pg++) {
      ...
      let r = await chamarBling(`https://api.bling.com.br/Api/v3/nfe?limite=100&pagina=${pg}&tipo=1`, { fundo: true });
      if (!r.ok && r.status === 429) {
        ...
        for (let tent = 1; tent <= 3 && !r.ok && r.status === 429; tent++) {
          ...
          r = await chamarBling(`https://api.bling.com.br/Api/v3/nfe?limite=100&pagina=${pg}&tipo=1`, { fundo: true });
        }
      }
