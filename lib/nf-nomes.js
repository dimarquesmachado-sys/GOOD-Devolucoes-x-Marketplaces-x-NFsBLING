...
module.exports = ({ chamarBling }) => {
  ...
  // b272 (Codex #213, P1) - ⚠️ MESMO CONSERTO DO ml-returns.js: `construirIndiceInterno`
  // guarda a falha HTTP em `IDX.erro` e RESOLVE (nao rejeita), entao o
  // `.catch()` original nunca via o 429 do boot. Agora o erro dentro do
  // resultado vira rejeicao aqui, na borda, pra alimentar o retry.
  //
  // b272 (Codex #213, P2) - timer de retry registrado na drenagem
  // (`daquiA`), nao `setTimeout` cru — mesmo motivo do ml-returns.js.
  function preAquecer(tentativa = 1) {
    ...
    return construirIndice({ fundo: true }).then((idx) => {
