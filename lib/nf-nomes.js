...
module.exports = ({ chamarBling }) => {
  ...
  async function construirIndiceInterno(opts = {}) {
    ...
    // b280.1 (Codex, P1) - ⚠️ TETO DE PAGINAS NAO E "COMPLETO".
    //
    // O passe curto do boot chama com `maxPaginas: 10` (b277) de proposito —
    // rapido e barato, mas so cobre ~17 dos 120 dias da janela. Se as 10
    // paginas esgotam SEM bater na data de corte nem no fim dos dados (a
    // ultima ainda veio CHEIA), ha mais NF alem do teto que este passe nao
    // leu — e isso e diferente de "varri tudo que existe".
    let limitadoPorTeto = false;
    ...
    for (let pg = 1; pg <= maxPaginas; pg++) {
      ...
      // b277 - publica a cada 3 paginas na primeira montagem, nao so na 3
      // e depois de 10 em 10. Com o passe curto em 10 paginas, publicar
      // apenas na 3 e na 10 deixava um vao longo — e cada publicacao ja
      // deixa a busca util pra quem chegar naquele momento.
      if (primeiraMontagem && (pg % 3 === 0 || pg % 10 === 0)) {
        IDX.mapa = { ...mapa };
      ...
      // b280.1 - chegou na ultima pagina permitida com a pagina ainda CHEIA:
      // o teto, nao a data nem o fim dos dados, e que interrompeu a varredura.
      if (pg === maxPaginas) limitadoPorTeto = true;
    ...
    // b276 (Codex, P1) - ⚠️ BUILD FRIO QUE FALHA VAZIO NAO E "COMPLETO".
    //
    // Se o 429 esgota as 3 retentativas (ou vem outro HTTP ruim) ja na
    // pagina 1, `erroBusca` fica marcado mas `totalNFs` continua 0 — o
    // 1o checkpoint de parcial (pagina 3) nunca chega a publicar nada.
    // Carimbar `ts` mesmo assim faria `montando` (`!IDX.ts`, b275) e
    // `completo` (`statusIndice`) mentirem que o indice esta pronto: a
    // rota engoliria o aviso que o #275 acabou de criar, e como `vencido`
    // tambem le `IDX.ts`, a busca por nome ficaria 30min repetindo "nao
    // encontrado" seco pro mesmo erro que so durou 1 pagina.
    //
    // Mesma regra que a AMB ja tem (nf-nomes-AMB.js, ml-returns-AMB.js):
    // erro + indice vazio NAO carimba `ts`, o proximo bipe tenta de novo
    // em vez de confiar num indice vazio por meia hora.
    const falhouVazio = !!erroBusca && totalNFs === 0;
    IDX.ts = falhouVazio ? 0 : Date.now();
    // b280.1 (Codex, P1) - ⚠️ SE O TETO LIMITOU, O INDICE CONTINUA PARCIAL.
    //
    // Sem isto, o passe curto do boot (10 paginas, b277) esgotava o teto com
    // a ultima pagina cheia, carimbava `ts` (b268: "agora esta COMPLETO") e
    // zerava `parcialAte` — o indice de ~17 dias passava a se anunciar como
    // os 120 dias inteiros. `montando` tambem mentiria (`!IDX.ts` vira
    // falso), entao o aviso que este PR criou ficava mudo bem no passe que MENOS
    // cobre, ate o passe completo (agendado em paralelo) terminar.
    if (falhouVazio) {
      // mantem o `parcialAte` anterior — varredura vazia nao e "completa"
    } else if (limitadoPorTeto) {
      IDX.parcialAte = paginasLidas;
    } else {
  ...
  function statusIndice() {
    return {
      ...
      // b276 - ⚠️ ha construcao rodando AGORA, e ha quanto tempo. Sem isto,
      // "nao montou" e "esta montando ha 6 minutos" eram indistinguiveis —
      // e a segunda e sintoma de travamento, nao de lentidao.
      construindo_ha_s: IDX.construindoDesde
        ? Math.round((Date.now() - IDX.construindoDesde) / 1000) : null,
  ...
  async function buscarPorNome(texto) {
    ...
    // b275 - ⚠️ O AVISO SUMIA JUSTAMENTE NO PIOR MOMENTO.
    //
    // `indiceParcial` so era marcado quando havia PARCIAL (`parcialAte`).
    // Mas logo apos um deploy o indice esta VAZIO — nem a primeira pagina
    // foi lida — e ai `parcialAte` e null: a tela mostrava "Codigo nao
    // encontrado" SECO, indistinguivel de "esse nome nao existe".
    //
    // Foi o que o dono viu agora: buscou "charles" logo apos o deploy do
    // #216 e levou 404 sem aviso nenhum, quando minutos antes o mesmo nome
    // trazia 5 NFs.
    //
    // ⚠️ VAZIO E PIOR QUE PARCIAL, e merecia MAIS aviso, nao menos.
    //
    // `montando` cobre os dois casos: parcial (algumas paginas) e vazio
    // (nenhuma). Quem le decide o texto; o que importa e a tela nao afirmar
    // "nao existe" quando a resposta e "ainda nao sei".
    const marcarParcial = (r) => ({
      ...r,
      indiceParcial: IDX.parcialAte || null,
      montando: !IDX.ts,   // sem carimbo de completo = ainda montando
    });
    ...
    if (vencido && !IDX.ts) {
      ...
      // vez de abrir a sua.
      // b276 - ⚠️ A CONSTRUCAO NAO PODE FICAR PENDURADA PRA SEMPRE.
      //
      // MEDIDO EM PRODUCAO (10/09): 6+ minutos de servico no ar e o
      // /health mostrava `paginas_lidas: null`, `erro: null`, `total: 0` —
      // ou seja, a varredura COMECOU e nunca chegou ao fim, e nem falhou.
      // O estoquista buscava "charles" e nao achava nada.
      //
      // A causa: o porteiro de ritmo pos a conta `good` em PAUSA (429), e
      // cada pagina fica esperando a vez. Nos fizemos ~22 chamadas em 6
      // min — bem abaixo do limite — entao a pausa vem do outro servico que
      // divide a conta. Esperamos junto.
      //
      // ⚠️ E O PIOR NAO E A DEMORA, E O `emConstrucao` PRESO: enquanto a
      // promessa nao resolve, TODA busca seguinte espera A MESMA promessa
      // pendurada. Uma varredura travada trava a busca inteira, pra sempre.
      //
      // Teto de 4 min na construcao: passou disso, ela e abandonada, o
      // `emConstrucao` limpa, e a proxima busca comeca uma nova. O que ela
      // ja tiver publicado como parcial continua valendo.
      ...
      if (!IDX.emConstrucao) {
        ...
        IDX.emConstrucao = Promise.race([
          ...
          new Promise((ok) => setTimeout(() => {
            console.warn(`[NF-NOMES] ⚠️ construcao passou de ${TETO_CONSTRUCAO_MS / 1000}s `
              + '— abandono e libero pra proxima tentar (provavel pausa do porteiro)');
            ok();
          }, TETO_CONSTRUCAO_MS)),
        ])
          .catch(() => {})
