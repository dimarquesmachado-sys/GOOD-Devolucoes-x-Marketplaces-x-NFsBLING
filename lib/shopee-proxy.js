...
async function acharDevolucao(codigo) {
  ...
  // v3.47 - FALLBACK 2 pro BR da etiqueta de INSUCESSO: o BR e o rastreio
  // do pedido ORIGINAL. O proxy mantem um indice tracking->pedido dos
  // cancelados recentes - bipar o BR resolve sozinho (sem digitar nada).
  if (!hit && alvoTok) {
    try {
      ...
      if (dT && dT.ok && dT.encontrado && dT.devolucao) {
        ...
        console.log(`[SHOPEE] ${alvoTok}: achou via INDICE tracking->pedido cancelado (insucesso)`);
      }
    } catch (e) { /* fallback silencioso */ }
  }
