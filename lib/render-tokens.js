...
/**
 * @param {Array|Function} updates  a lista, OU uma funcao que a devolve.
 *
 * b250.5 (Codex, P1) - ACEITA FUNCAO porque o valor precisa ser lido DENTRO
 * da fila, nao antes dela.
 *
 * A migracao de env vars monta a lista a partir de `process.env`. Se ela
 * montar antes de entrar na fila e uma rotacao de token estiver na frente,
 * a rotacao atualiza o refresh e a migracao grava por cima o valor VELHO —
 * e refresh consumido mata a integracao no proximo restart.
 *
 * Passando funcao, ela e chamada quando chega a vez: le o estado do momento.
 */
async function atualizarTokensNoRender(updates) {
  const resolver = () => (typeof updates === 'function' ? updates() : updates);
  const minhaVez = filaRenderGlobal.then(
    () => escreverNoRender(resolver()),
    () => escreverNoRender(resolver()),   // erro do anterior nao trava a fila
  );
