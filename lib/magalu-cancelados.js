...
const CLASSES_COM_NF = ['pago_cancelado_com_nf', 'estornado_apos_envio'];

function primeiraNota(linha) {
  const notas = Array.isArray(linha && linha.notas) ? linha.notas : [];
  return notas.find((n) => n && (n.chave || n.numero)) || null;
}
...
function normalizar(linha, empresa) {
  ...
  if (CLASSES_COM_NF.indexOf(classe) === -1) return null;

  const nota = primeiraNota(linha);
  ...
  return {
    ...
    nf_numero: (nota && nota.numero) || null,
...
async function buscar(empresa, opcoes) {
  ...
  const itens = linhas.map((l) => normalizar(l, emp)).filter(Boolean);

  return { ok: true, itens, total_bruto: linhas.length };
}

module.exports = { normalizar, buscar, CLASSES_COM_NF };
