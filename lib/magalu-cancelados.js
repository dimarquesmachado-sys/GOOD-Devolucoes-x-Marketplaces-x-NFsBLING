...
const ACAO_POR_CLASSE = {
  ...
  // ⚠️ b195.2 - `entrada_estoque: TRUE`, e isto foi correcao DELE.
  //
  // Eu tinha posto false achando que dar entrada duplicaria o inventario.
  // Nao duplica: a NF DE VENDA JA DEU BAIXA quando foi emitida, entao o
  // sistema acha que o produto saiu enquanto ele esta no CD. A entrada
  // CORRIGE essa diferenca.
  //
  // [stated] "nos casos q o produto nunca foi postado, é só gerar devolução
  // normal, e depósito Geral. Simples."
  //
  // Eu corrigi o comentario em b193 mas esqueci o VALOR aqui — entao a tela
  // continuava mandando usar o deposito de DEFEITO nesses casos.
  nf_sem_saida: { entra: true, entrada_estoque: true, pode_cancelar: true },
...
function numeroDaChave(chave) {
  ...
  // b194.1 (Codex): so confio na chave se ela PARECE uma NF-e de verdade.
  //
  // Chave de teste ou lixo de 44 digitos ('3' repetido, por exemplo) daria
  // um numero inventado — e agora ela tem precedencia sobre o campo da API,
  // entao um numero usavel seria descartado por causa de lixo.
  const mm = parseInt(d.slice(4, 6), 10);       // mes da competencia
  const mod = d.slice(20, 22);                  // 55 = NF-e, 65 = NFC-e
...
function normalizarTodas(linha, empresa) {
  ...
  return notas.map((nota, i) => montar(linha, nota, classe, empresa, i));
}
...
function montar(linha, nota, classe, empresa, posicao) {
  ...
  // b195 - ⚠️ CORRECAO DO DIAGNOSTICO ANTERIOR.
  //
  // Em b194 eu conclui que "a Magalu manda um numero PROPRIO dela". ERRADO.
  // O dono trouxe o JSON real da AMB e os numeros BATEM com a chave:
  //   numero 637 <- chave ...0000006371757802116 (nNF 000000637) ✓
  //   numero 822 <- chave ...0000008221313213710 ✓
  //   numero 377 <- chave ...0000003771441283894 (serie 003) ✓
  //
  // O que me enganou: comparei as notas da AMB (numeracao baixa, series
  // 001 e 003) com as da GOOD (casa dos setenta mil) e assumi que o campo
  // estava errado. Sao EMPRESAS diferentes, com numeracao propria.
  //
  // A chave continua tendo precedencia — ela e o documento fiscal e cobre
  // o caso de a API omitir o numero. Mas o "nao localizada no Bling" tem
  // OUTRA causa, ainda nao diagnosticada: pode ser que essas notas da
  // Magalu simplesmente nao estejam no Bling (emitidas pelo marketplace,
  // nao por nos).
  ...
  return {
    ...
    // b194.1 (Codex): o ID vem da CHAVE, nao do numero.
    //
    // Corrigir o numero (b194) mudava o id junto — e os casos que o dono JA
    // registrou tem o id ANTIGO gravado em `[caso:X]`. Eles ficariam orfaos:
    // o painel nao reconheceria o registro e o caso voltaria a aparecer como
    // pendente, com risco de segunda NF.
    //
    // A chave nao muda nunca, e distingue ate notas de SERIES diferentes com
    // o mesmo nNF — que o numero sozinho nao faz.
    // b194.2 (Codex): sem chave E sem numero, duas notas do mesmo pedido
    // ficariam com o id IDENTICO (so o pedido) e uma sobrescreveria a outra
    // em qualquer mapa. Uso a posicao como ultimo recurso — feia, mas
    // distingue, e o alternativo e perder um caso.
    id: String(linha.order_id || linha.order_code || '')
      + ((nota && nota.chave)
        ? '#' + String(nota.chave).replace(/\D/g, '').slice(-10)
        : (numeroDaNota
          ? '#' + numeroDaNota
          : (posicao != null ? '#n' + posicao : ''))),
    // b194.2 (Codex): o id ANTIGO, pra casar registros feitos antes desta
    // correcao. Havia DOIS formatos antigos, e eu so cobria um:
    //   - com `numero` na API  -> pedido#numeroDaApi
    //   - so com chave         -> pedido#<8 ultimos digitos da chave>
    // O segundo ficaria orfao, que e justamente o que este campo veio
    // evitar. Devolvo os dois quando existirem.
    // b194.3 (Codex): nota que so tinha CHAVE gerava o id pelo NUMERO
    // extraido dela, nao pelo final da chave.
    //
    // A formula antiga era `pedido#numeroDaNota`, e `numeroDaNota` ja saia
    // da chave quando nao havia `numero` na API. Entao o registro velho tem
    // `P1#76466` — e eu estava oferecendo `P1#35887584`, que nenhum dos dois
    // checks reconhece. O caso voltaria como pendente, com risco de segunda
    // NF: exatamente o problema que este campo veio evitar.
    //
    // Terceira vez que erro NESTE campo. Os formatos antigos possiveis sao:
    //   com `numero` na API -> pedido#numeroDaApi
    //   so com chave        -> pedido#nNFdaChave
    // Devolvo os dois, mais o final da chave por seguranca.
    id_legado: (nota && nota.numero)
      ? String(linha.order_id || linha.order_code || '') + '#' + nota.numero
      : (numeroDaNota
        ? String(linha.order_id || linha.order_code || '') + '#' + numeroDaNota
        : undefined),
    id_legado2: (nota && nota.numero && numeroDaNota && numeroDaNota !== nota.numero)
      ? String(linha.order_id || linha.order_code || '') + '#' + numeroDaNota
      : ((nota && nota.chave)
        ? String(linha.order_id || linha.order_code || '') + '#' + String(nota.chave).slice(-8)
        : undefined),
    marketplace: 'magalu',
