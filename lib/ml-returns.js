...
module.exports = ({ chamarML }) => {
  ...
  async function construirIndice(opts = {}) {
    ...
    for (let pg = 0; pg < maxPaginas; pg++) {
      const r = await chamarML(`https://api.mercadolibre.com/post-purchase/v1/claims/search?offset=${pg * 30}&limit=30`);
      if (!r.ok) { IDX.erro = `claims/search HTTP ${r.status}`; break; }
