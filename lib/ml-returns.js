...
module.exports = ({ chamarML }) => {
  ...
  function resumoEspreita() {
    ...
    const emTransito = [], aguardando = [];
    let entregues = 0;
    for (const d of Object.values(IDX.mapa)) {
      const st = String(d.status_devolucao || '').toLowerCase();
      if (st === 'delivered') { entregues++; continue; }
      if (st === 'label_generated') { aguardando.push(d); continue; }
      if (EM_TRANSITO.includes(st)) {
        emTransito.push({ marketplace: 'ml', pedido: d.order_id, tracking: d.tracking, status: st, dias_em_transito: dias(d.claim_date), claim_id: d.claim_id, status_money: d.status_money || null });
      }
    }
    emTransito.sort((x, y) => (y.dias_em_transito || 0) - (x.dias_em_transito || 0));
    return { quente: IDX.ts > 0, em_transito: emTransito, aguardando_postagem: aguardando.length, entregues_indice: entregues };
  }
