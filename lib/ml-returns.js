...
module.exports = ({ chamarML }) => {
  ...
  async function construirIndice(opts = {}) {
    ...
    const filtroBase = uid
      ? `players.user_id=${uid}&players.role=respondent&range=date_created:after:${desde}&sort=date_created:desc`
      : `status=opened`; // fallback se /users/me falhar
    for (let pg = 0; pg < maxPaginas; pg++) {
      const r = await chamarML(`https://api.mercadolibre.com/post-purchase/v1/claims/search?${filtroBase}&offset=${pg * 30}&limit=30`);
      if (!r.ok) { erroBusca = `claims/search HTTP ${r.status}: ${JSON.stringify(r.error || '').slice(0, 120)}`; break; }
      const lista = r.data?.data || [];
      if (lista.length === 0) break;
      for (const c of lista) claims.push({ id: c.id, resource: c.resource, resource_id: c.resource_id, status: c.status, stage: c.stage, date: c.date_created });
