...
module.exports = ({ chamarML }) => {
  ...
  async function construirIndice(opts = {}) {
    ...
    try {
      ...
      if (drenagem.ehCancelamento(e)) {
        console.log(`[ML-RETURNS] ${e.message} — indice NAO publicado`);
        return;
      }
      throw e;
    }
  ...
  async function construirIndiceInterno(opts = {}) {
    ...
    for (const st of ['opened', 'closed']) {
      for (let pg = 0; pg < maxPaginas; pg++) {
        // b263 - mesma coisa: a varredura de devolucoes do ML continuaria
        // paginando durante o deploy, no processo que ja recebeu o aviso.
        drenagem.pontoDeCancelamento(true, 'ml-returns');
    ...
    for (let i = 0; i < claims.length; i += 3) {
      // b263.1 (Codex, P1) - ⚠️ A FASE 2 TAMBEM PARA.
      //
      // Meu `break` na paginacao saia so do laco INTERNO. Depois dele a
      // funcao entrava aqui e chamava `/returns` pra CADA claim ja
      // coletado — com retry e possivel segunda rodada. Uma unica pagina
      // coletada vira ~30 chamadas DEPOIS da drenagem comecar.
      //
      // Ou seja: eu preservava exatamente o consumo de cota e a
      // sobreposicao de credencial que esta mudanca veio impedir.
      drenagem.pontoDeCancelamento(true, 'ml-returns');
      ...
      await Promise.all(lote.map(async (c) => {
        try {
          ...
          // v4.18 - uma devolucao que passa pelo CD do ML tem DOIS trechos:
          // cliente->CD e depois CD->galpao. O trecho do CD ja cumpriu o papel
          // quando existe o trecho pro seller - antes ele ficava preso no
          // indice como "em revisao no CD" pra sempre.
          const temTrechoPraNos = ships.some(x => x?.destination?.name === 'seller_address');
          for (const sh of ships) {
            ...
            mapa[trk] = {
              ...
              superado: (sh?.destination?.name === 'warehouse' && temTrechoPraNos), // v4.18
              status_claim: c.status || null,   // opened | closed
              stage_claim: c.stage || null,
              entregue_em: (sh?.status === 'delivered') ? (rr.data?.last_updated || null) : null, // v3.88: data REAL da entrega
  ...
  function resumoEspreita() {
    ...
    for (const d of Object.values(IDX.mapa)) {
      // v4.18 - trecho ja superado (o pacote seguiu do CD pra ca por outro
      // envio): nao e mais "em transito", e historico.
      if (d.superado) continue;
      ...
      if (st === 'delivered') {
        ...
        // v4.18 - chegou no CD e a reclamacao JA FECHOU: o ML resolveu por
        // dentro (revisou e recolocou a venda, ou ja despachou). Nao esta
        // vindo pra ca - antes aparecia como "em transito ha 118 dias".
        ...
        // b236.8 (Codex): `status_money` FALTAVA nas entregues — as duas
        // listas de transito (linhas acima) ja levavam. Sem ele o card de
        // alerta calculava `dinheiro` sempre null, e minha "paridade" do
        // b236.7 era so de fachada: o campo existia e nunca tinha valor.
        entreguesLista.push({ marketplace: 'ml', pedido: d.order_id, tracking: d.tracking, dias_desde: dias(d.entregue_em || d.claim_date), entregue_em: d.entregue_em || null, claim_id: d.claim_id, status_money: d.status_money || null, shipment_devolucao: d.shipment_devolucao || null });
        continue;
      }
  ...
  // b271 - ⚠️ FALHOU, TENTA DE NOVO. Antes era `catch` e pronto: um 429 no
  // boot (comum, porque a cota do Bling e DA CONTA e o Mover-Pedidos
  // tambem consome) deixava a espreita vazia por 25 MINUTOS, ate o proximo
  // ciclo.
  //
  // E espreita vazia tem consequencia na TELA: e ela que marca com ESTRELA
  // qual das NFs da busca por nome e a devolucao que esta a caminho. Sem
  // cache, nenhum card ganha estrela — foi exatamente o que o dono viu
  // ("cade a estrela?").
  //
  // 3 tentativas com espera crescente (30s, 60s, 120s). ⚠️ Espera longa de
  // proposito: se a causa foi 429, tentar logo de novo so piora.
  //
  // ⚠️ (Codex, PR #213) `construirIndice()` RESOLVE mesmo quando a busca
  // falhou — o HTTP ruim vira `IDX.erro`, nao rejeicao (linha ~247). O
  // `.catch()` sozinho nunca via o 429 que motivou esta funcao. Agora o
  // `.then()` inspeciona o indice publicado e transforma erro em rejeicao,
  // pra cair no MESMO caminho de retry.
  //
  // `aoSucesso` e opcional: quem precisa saber quando o indice ficou
  // pronto de verdade (ex.: o snapshot da espreita em server.js, que sem
  // isso podia rodar ANTES do indice do ML terminar de montar) passa um
  // callback aqui em vez de inventar outro cron.
  function preAquecer(tentativa = 1, aoSucesso) {
    construirIndice().then((idx) => {
      ...
      console.error(`[ML-RETURNS] pre-aquecimento falhou (tentativa ${tentativa}/3):`, e.message);
      if (tentativa >= 3) {
        console.error('[ML-RETURNS] ⚠️ desisto — a espreita fica vazia ate o proximo '
          + 'ciclo (25 min), e a ESTRELA da busca por nome nao vai aparecer.');
        return;
      }
      const espera = 30000 * Math.pow(2, tentativa - 1);
      console.log(`[ML-RETURNS] tento de novo em ${espera / 1000}s`);
      // ⚠️ (Codex) setTimeout cru nao e cancelado pela drenagem: um SIGTERM
      // no meio da espera deixava este timer vivo pra acordar o processo
      // VELHO durante o deploy. `daquiA` registra e cancela junto.
      drenagem.daquiA(() => preAquecer(tentativa + 1, aoSucesso), espera);
    });
  }
