...
module.exports = ({ chamarML }) => {
  ...
  //
  // b272 (Codex #213, P1) - ⚠️ O `.catch()` SO PEGAVA REJEICAO DE PROMISE.
  // Um 429 no `claims/search` NAO rejeita — `construirIndiceInterno` guarda
  // o erro em `IDX.erro` e RESOLVE normalmente (pra nao perder o que ja
  // coletou). Resultado: a retentativa nunca disparava exatamente no caso
  // que ela foi escrita pra cobrir. Agora o sucesso "com erro dentro" vira
  // rejeicao aqui, na borda, pra alimentar o mesmo mecanismo de retry.
  //
  // b272 (Codex #213, P2) - ⚠️ TIMER DE RETRY REGISTRADO NA DRENAGEM. Um
  // `setTimeout` cru sobrevive ao SIGTERM: se o processo comecar a sair
  // durante a espera, a retentativa dispara no deploy VELHO. `daquiA` ja
  // cancela isso (mesmo motivo do b264 pros loops de varredura).
  function preAquecer(tentativa = 1) {
    return construirIndice().then((idx) => {
      ...
      return new Promise((resolve) => {
        drenagem.daquiA(() => resolve(preAquecer(tentativa + 1)), espera);
      });
    });
  }
