...
/**
 * b276 - ⚠️ QUEM ENTRA NA FILA PRECISA PODER SAIR.
 *
 * Antes esta promessa NUNCA rejeitava: se o porteiro puser a conta em
 * pausa longa (429 de outro servico que divide a conta), quem esperava
 * ficava pendurado pra sempre.
 *
 * MEDIDO EM PRODUCAO (10/09): a construcao do indice de nomes ficou 6+ min
 * sem terminar e sem falhar — `paginas_lidas: null`, `erro: null`. E como
 * o `emConstrucao` guarda essa promessa, TODA busca seguinte esperava a
 * mesma coisa travada. A busca por nome parou de funcionar inteira.
 *
 * ⚠️ O teto e generoso (2 min) de proposito: pausa de 429 e legitima e
 * esperar e o certo. O que nao pode e esperar SEM FIM — passou disso, quem
 * chamou recebe erro e decide o que fazer (a varredura de fundo desiste e
 * tenta depois; a busca do estoquista responde com o que tem).
 */
function aguardarVez(opcoes = {}) {
  const teto = Number(opcoes.tetoMs || process.env.BLING_TETO_FILA_MS || 120000);
  return new Promise((liberar, falhar) => {
    ...
    const t = setTimeout(() => {
      if (cracha()) return;
      const e = new Error(`espera na fila do Bling passou de ${teto / 1000}s `
        + '(provavel pausa do porteiro por 429)');
      e.filaEstourou = true;
      falhar(e);
    }, teto);

    (opcoes.fundo ? filaDeFundo : filaInterativa).push(() => {
      if (cracha()) return;   // ja estourou; a vaga vai pro proximo
      clearTimeout(t);
      liberar();
    });
    despachar();
