...
module.exports = ({ atualizarTokensNoRender }) => {
  ...
  const SCOPES = (process.env.MAGALU_SCOPES || [
    ...
    'open:order-logistics-seller:read',      // v3.60 - LOGISTICS do pedido
    'open:logistic-carrier-shippings:read',  // v3.60 - API de Remessas (o 196634440?)
  ].join(' '));
  ...
  async function construirIndiceDevolucoes(opts = {}) {
    ...
    // 2) reverse_code so dos tickets ABERTOS (fechados nao tem pacote vindo)
    ...
    IDX.erro = null;
    console.log(`[MAGALU] indice: ${total} tickets, ${comReversa} com remessa reversa, em ${IDX.duracaoSeg}s`);
    return IDX;
  }
  ...
  async function acharDevolucao(codigo) {
    ...
    if (!IDX.ts || (Date.now() - IDX.ts) > 30 * 60000) {
      try { await construirIndiceDevolucoes(); } catch (e) { /* segue com o que tiver */ }
    }
