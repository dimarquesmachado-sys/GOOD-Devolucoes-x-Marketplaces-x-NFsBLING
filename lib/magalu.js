      ...
      for (let i = 0; i < abertos.length; i += 4) {
        ...
        await Promise.all(lote.map(async (dev) => {
          try {
            ...
            let rc = ordenadas.map(codigoDe).find(Boolean) || null;

            // b190 - se o /returns nao deu o codigo, procuro no TEXTO das
            // mensagens. Custa uma chamada a mais, e so pros tickets que
            // ficariam sem codigo de qualquer forma.
            if (!rc) {
              try {
