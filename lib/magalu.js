...
module.exports = ({ atualizarTokensNoRender }) => {
  ...
  async function trocarCodePorTokens(code) {
    ...
    const gravarAuth = [
      ...
      // b274 - so carimba se persistiu de fato
      const persistiuAuth = await atualizarTokensNoRender(gravarAuth);
      if (persistiuAuth && PREVENTIVA) PREVENTIVA.marcarRenovado();
      if (!persistiuAuth) console.warn('[MAGALU] tokens ativos na MEMORIA, mas nao persistidos no Render');
  ...
  // b190 - SEGUNDA FONTE DO CODIGO: o texto das mensagens do protocolo.
  //
  // Descoberto pela conversa do Checkout num caso real: quando a coleta e
  // reagendada, o /returns pode nao trazer o codigo, mas o SAC escreve o
  // numero na mensagem, assim:
  //
  //   "O número da coleta a ser realizado é 4667981503
  //    - Nº do objeto: DA597697016BR"
  //
  // O escopo open:ticket-messages-seller:read ja estava pedido.
  async function mensagensDoTicket(ticketId) {
    return chamarMagalu(`/seller/v0/tickets/${encodeURIComponent(ticketId)}/messages`);
  }

  /**
   * Garimpa um codigo de rastreio dos Correios no texto livre.
   *
   * O formato e fixo: 2 letras + 9 digitos + 2 letras (DA597697016BR).
   * Procuro so esse padrao — nao invento outros, porque numero solto no
   * meio de texto de SAC pode ser qualquer coisa (protocolo, pedido, NF).
   */
  function codigoNoTexto(texto) {
    // b190.4 (Codex): o SAC nem sempre escreve em MAIUSCULA. Aceito
    // qualquer caixa e devolvo em maiuscula, que e como os Correios usam e
    // como o resto do sistema indexa.
      ...
      for (let i = 0; i < abertos.length; i += 4) {
        ...
        await Promise.all(lote.map(async (dev) => {
          try {
            ...
            // b190 - CASO REAL QUE ESCAPOU (GOOD, pedido 1540670112009168,
            // lustre de R$ 1.299,90): a Magalu agendou coleta nos Correios,
            // objeto DA597697016BR, coleta 466.798.1503. A cliente reclamou
            // que ninguem foi buscar, a coleta foi REAGENDADA, e so
            // aconteceu dias depois — coletado 05/08, entregue no galpao
            // 06/08. Nossa consulta respondia tem_reversa:false.
            //
            // O pacote ficou tres semanas no galpao sem ninguem esperando,
            // que e exatamente o que o "a espreita" existe pra evitar.
            //
            // Duas causas possiveis, e trato as duas:
            //   1. com reagendamento ha VARIAS remessas, e a que vale pode
            //      nao ser a primeira — pego a que tiver codigo, em qualquer
            //      posicao, preferindo a mais recente
            //   2. o codigo pode vir com outro nome no JSON
            const CAMPOS_CODIGO = ['reverse_code', 'tracking_code', 'object_code',
                                   'tracking', 'code', 'reverse_tracking_code'];
            const codigoDe = (x) => {
            ...
            // a mais RECENTE primeiro: com reagendamento, a ultima e a que valeu
            const ordenadas = res.slice().sort((a, b) =>
              String(b?.created_at || b?.date || '').localeCompare(String(a?.created_at || a?.date || '')));

            // b190.1 (Codex): o codigo da MAIS RECENTE e o que vale.
            //
            // Se a ultima remessa (a reagendada, que efetivamente aconteceu)
            // nao traz codigo, pegar o de uma tentativa ANTERIOR que falhou
            // e pior que nao ter nenhum: seria um rastreio obsoleto, de um
            // pacote que nunca saiu. Melhor ir nas mensagens.
            const rcMaisRecente = ordenadas.length ? codigoDe(ordenadas[0]) : null;
            let rc = rcMaisRecente;
            const rcAntigo = ordenadas.map(codigoDe).find(Boolean) || null;

            // se o /returns nao deu o codigo DA MAIS RECENTE, procuro no
            // TEXTO das mensagens. Custa uma chamada a mais, so pros tickets
            // que ficariam com codigo obsoleto ou sem nenhum.
            if (!rc) {
              try {
                const mm = await mensagensDoTicket(dev.ticket_id);
                const msgs = (mm.ok && mm.data?.results) ? mm.data.results : [];

                // b190.3 (Codex): a mensagem MAIS RECENTE manda, pelo mesmo
                // motivo da remessa. Num ticket reagendado, o SAC escreve o
                // objeto da coleta que falhou e depois o da que valeu — ler
                // na ordem de chegada pegaria o obsoleto.
                const porData = msgs.slice().sort((a, b) =>
                  String(b?.created_at || b?.date || b?.sent_at || '')
                    .localeCompare(String(a?.created_at || a?.date || a?.sent_at || '')));

                for (const msg of porData) {
                  const achado = codigoNoTexto(msg?.body || msg?.message || msg?.text || '');
                  if (achado) { rc = achado; dev.codigo_da_mensagem = true; break; }
                }
              } catch (e) { /* segue sem codigo; melhor que derrubar a fase 2 */ }

              // ultimo recurso: o codigo antigo. Melhor um rastreio velho
              // que nenhum — mas marcado, pra quem ler saber que pode ser de
            ...
            if (rc) {
              ...
              // b190.1 (Codex): acharDevolucao() LIMPA o codigo (so digitos)
              // antes de procurar. Indexando so 'R:DA597697016BR', a busca
              // por 'R:597697016' nao acharia — ou seja, o caso que este
              // conserto veio resolver continuaria escapando.
              // b190.3 (Codex): a chave so-digitos pode COLIDIR — DA597697016BR
              // e XY597697016ZW viram o mesmo '597697016'. Guardo, porque a
              // busca limpa o codigo antes de procurar, mas NAO sobrescrevo
              // uma entrada existente: na duvida, a primeira fica, e o codigo
              // completo continua indexado e resolve sem ambiguidade.
              const soNum = String(rc).replace(/\D/g, '');
              if (soNum && soNum !== dev.reverse_code && !IDX.mapa['R:' + soNum]) {
                IDX.mapa['R:' + soNum] = dev;
              }
              comReversa++;
            }
