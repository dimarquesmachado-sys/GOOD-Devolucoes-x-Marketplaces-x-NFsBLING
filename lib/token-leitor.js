...
async function lerTokenDetalhado(empresa, integracao) {
  ...
  anotar(empresa, integracao, 'estado', r.estado === 'ok' && r.do_cache ? 'ok_cache' : r.estado);
  if (r.estado !== 'ok') eixoMetrica(empresa, integracao).ultima_falha = new Date().toISOString();
  else eixoMetrica(empresa, integracao).ultimo_sucesso = new Date().toISOString();
  return r;
}
