...
async function lerTokenDetalhado(empresa, integracao) {
  // b259.4 (Codex, P2) - ⚠️ A ANOTACAO ACONTECE NO PONTO UNICO DE SAIDA.
  //
  // Eu vinha anotando saida por saida e ESQUECI TRES (`nao_configurado`,
  // e os `ausente`/`nao_implementado` vindos do cache). O Codex apontou
  // duas vezes a mesma classe — e ia continuar apontando, porque cada
  // saida nova que eu escrevesse nasceria sem anotacao.
  //
  // Envolvo a funcao: ela devolve o resultado, e o embrulho anota. Saida
  // nova entra coberta por construcao, sem eu precisar lembrar.
  const t0 = Date.now();
  const r = await lerTokenBruto(empresa, integracao);
  const m = eixoMetrica(empresa, integracao);
  const doCache = r.estado === 'ok' && r.do_cache;

  anotar(empresa, integracao, 'estado', doCache ? 'ok_cache' : r.estado);

  // ⚠️ b259.5 (Codex, P2): SO O SUCESSO REMOTO carimba `ultimo_sucesso`.
  //
  // O cache carimbava tambem — entao, com o dono FORA e o cache quente, o
  // /health mostraria "sucesso ha 3 segundos" enquanto ninguem falava com
  // ele ha 5 minutos. Isso destruiria a separacao remoto/cache que eu
  // acabei de criar: o campo mais confiavel do painel viraria o mais
  // enganoso.
  if (r.estado === 'ok' && !doCache) {
    m.ultimo_sucesso = new Date().toISOString();
    anotar(empresa, integracao, 'latencia', Date.now() - t0);
  } else if (doCache) {
    m.ultimo_do_cache = new Date().toISOString();   // separado, e honesto
  }

  // ⚠️ E `ausente`/`nao_implementado` NAO SAO FALHA — achei auditando a
  // funcao inteira, nao veio de apontamento.
  //
  // 404 = a empresa nao tem essa integracao (GOOD/tiktok e o caso real);
  // 501 = o dono ainda nao tem leitor. Os dois sao resposta CORRETA e
  // definitiva. Carimbar como falha deixaria o painel parecendo degradado
  // pra sempre num eixo que nunca vai existir — e ai ninguem olha mais.
  const ehFalhaDeVerdade = !['ok', 'ausente', 'nao_implementado'].includes(r.estado);
  if (ehFalhaDeVerdade) {
    m.ultima_falha = new Date().toISOString();
    // a latencia da falha importa MAIS que a do sucesso: dono lento e o
    // sintoma que antecede o timeout
    anotar(empresa, integracao, 'latencia', Date.now() - t0);
  }
  return r;
}

async function lerTokenBruto(empresa, integracao) {
  ...
  try {
    ...
    // b259.3 (Codex, P2): resposta FORA do conjunto tratado tambem conta.
    // Com `validateStatus: () => true`, um 418/302/qualquer coisa vinha
    // parar aqui e saia sem anotar nada — o dono podendo responder lixo
    // sem aparecer em `estados`.
...
function eixoMetrica(empresa, integracao) {
  ...
  if (!metricas[k]) {
    metricas[k] = {
      ...
      // b329 (Codex, P2) - ⚠️ O RETRY E GUARDADO POR STATUS DE ORIGEM.
      //
      // Um total agregado (`{ ok, falhou }`) nao diz QUAL invalidacao aquele
      // retry resolveu. Com 401 e 403 acumulando no mesmo eixo ao longo do
      // tempo, um 401 respondido nao "quita" um 403 pendente nem vice-versa
      // — mas o total agregado deixava os dois se cobrirem, e o saldo por
      // status (usado pra distinguir token vencido de permissao) ficava
      // errado sempre que os dois status coexistissem na mesma janela.
      retry_apos_invalidar: {
        por_401: { ok: 0, falhou: 0 },
        por_403: { ok: 0, falhou: 0 },
        outras: { ok: 0, falhou: 0 },
      },
 ...
 * `status` e o status QUE GEROU a invalidacao (401/403/outro) — precisa ser
 * o mesmo que foi passado a `anotarInvalidacao` na mesma chamada, senao o
 * saldo por status em `vereditoDeCorte` atribui o retry ao status errado.
 */
function anotarRetry(empresa, integracao, deuCerto, status) {
  const m = eixoMetrica(empresa, integracao);
  const grupo = status === 401 ? m.retry_apos_invalidar.por_401
    : status === 403 ? m.retry_apos_invalidar.por_403
    : m.retry_apos_invalidar.outras;
  if (deuCerto) grupo.ok++;
  else grupo.falhou++;
}
...
function vereditoDeCorte() {
  ...
  for (const eixo of ['good/bling', 'good/ml']) {
    ...
    // ⚠️ a frase muda conforme o caso: "so cache" so faz sentido SE houve
    // cache. Sem isso, quem le "nunca leu do dono (so cache)" com zero
    // cache acha que o painel esta quebrado — e para de confiar nele.
    if (m.fonte.remoto === 0) {
      motivos.push(m.fonte.cache > 0
        ? `nunca leu do dono nesta janela (${m.fonte.cache}x so do cache)`
        : 'nunca leu do dono');
    }
    const retryPor401 = m.retry_apos_invalidar.por_401;
    const retryPor403 = m.retry_apos_invalidar.por_403;
    const retryOutras = m.retry_apos_invalidar.outras;
    const retryFalhouTotal = retryPor401.falhou + retryPor403.falhou + retryOutras.falhou;
    if (retryFalhouTotal > 0) motivos.push(`retry falhou ${retryFalhouTotal}x`);

    // ⚠️ b259.3 (Codex, P1): INVALIDACAO SEM RETRY CONHECIDO TAMBEM BLOQUEIA.
    //
    // As chamadas `semRetentativa` (detalhe de produto, em volume) tomam
    // 401, invalidam o cache e SAEM — sem retry, entao sem resultado
    // registrado. Eu ignorava `invalidacoes` no veredito, e o eixo aparecia
    // pronto mesmo levando 401 o dia todo por esse caminho.
    const invTotal = m.invalidacoes.por_401 + m.invalidacoes.por_403 + m.invalidacoes.outras;
    const retryPor401Total = retryPor401.ok + retryPor401.falhou;
    const retryPor403Total = retryPor403.ok + retryPor403.falhou;
    const retryOutrasTotal = retryOutras.ok + retryOutras.falhou;
    const retryTotal = retryPor401Total + retryPor403Total + retryOutrasTotal;
    if (invTotal > retryTotal) {
      // ⚠️ b329 - DIZ SE O SALDO E POR 401 OU POR 403: a conduta muda.
      //
      // O contrato v7 diz que o ML responde 403 com TOKEN VENCIDO; a v10
      // refinou: 403 de ROTA RESTRITA AO DONO DO RECURSO nao indica token,
      // e renovar nao cura (o /health ja expoe `ml_403` com essa nota).
      //
      // 📌 Os dois chegam aqui como "invalidacao sem retry" e o dono nao
      // distingue — mas so UM tem conserto pelo token. Sem isso, um eixo
      // pode ficar bloqueado pra sempre por 403 de permissao.
      //
      // ⚠️ NAO afrouxo o veredito: o bloqueio continua. So digo QUAL e o
      // saldo, pra decisao ser informada em vez de adivinhada.
      //
      // b329.1 (Codex, P2) - ⚠️ O SALDO E POR STATUS, NAO O TOTAL BRUTO.
      //
      // Comparar `por_403 > 0 && por_401 === 0` usava os contadores CRUS,
      // acumulados desde `DESDE`. Um 401 respondido com sucesso ainda
      // deixava `por_401` > 0 pro resto da janela — entao um 403 pendente
      // de verdade nunca era classificado como "so 403" se algum 401
      // qualquer, ja resolvido, tivesse acontecido antes. E o inverso
      // tambem: retries de 403 cobriam `invTotal - retryTotal` sem provar
      // que o 401 pendente estava coberto. O saldo certo e por status: o
      // que sobrou DEPOIS de tirar o retry daquele mesmo status.
      const saldo401 = m.invalidacoes.por_401 - retryPor401Total;
      const saldo403 = m.invalidacoes.por_403 - retryPor403Total;
      const saldoOutras = m.invalidacoes.outras - retryOutrasTotal;
      const so403 = saldo403 > 0 && saldo401 === 0 && saldoOutras === 0;
      motivos.push(`${invTotal - retryTotal} invalidacao(oes) sem retry conhecido `
        + '(o caminho semRetentativa sai sem repetir)'
        + (so403
          ? ' — ⚠️ TODAS por 403: pode ser rota restrita ao dono do recurso, '
            + 'que renovacao NAO cura (ver `ml_403` no /health)'
          : ` — por_401=${saldo401}, por_403=${saldo403}, outras=${saldoOutras}`));
    }
    if ((m.estados.config_invalida || 0) > 0) motivos.push('houve config_invalida');

    // ⚠️ achei auditando: `nao_configurado` nao entrava no veredito. Se a
    // `ADMIN_TOKEN_LEITURA_KEY` sumir do Render, o eixo cai no local e o
    // veredito diria so "caiu no local N vezes" — sem apontar que a causa
    // e configuracao faltando, que e a unica com conserto de 1 minuto.
    if ((m.estados.nao_configurado || 0) > 0) {
      motivos.push('ADMIN_TOKEN_LEITURA_KEY ausente (nem chegou a chamar o dono)');
    }

    // ⚠️ e `ausente`/`nao_implementado` em VOLUME merece aviso: nao e
    // falha, mas se o eixo so devolve isso, ele nao existe de verdade — e
    // "cortar" um eixo inexistente e sem sentido.
    const so404 = (m.estados.ausente || 0) + (m.estados.nao_implementado || 0);
    if (so404 > 0 && m.fonte.remoto === 0) {
      motivos.push(`o dono nunca teve token pra este eixo (${so404}x ausente/nao_implementado)`);
    }
...
function diagnostico() {
  ...
  return {
    ...
    // ⚠️ b259.6 (Codex, P1) - `pronto_pra_cortar` SAIU. Ele afirmava
    // PRONTO olhando so a LIGACAO (o codigo chama o leitor?), enquanto o
    // `pode_cortar` logo abaixo dizia NAO com base na EVIDENCIA. Dois
    // campos vizinhos no mesmo JSON, com instrucoes opostas — quem lesse o
    // primeiro cortaria um eixo que o segundo recusa.
    //
    // Agora ha UMA fonte por pergunta:
    //   `eixos`       -> a LIGACAO existe? (o codigo consulta o dono?)
    //   `pode_cortar` -> a EVIDENCIA permite? (o veredito)
    //
    // O nome antigo tambem prometia demais: "pronto" era so fiacao.
    ...
    // `ainda_local` saiu junto: dizia o mesmo que `eixos`, com outro nome.
    // Campo duplicado num painel e a mesma armadilha do contrato — duas
    // fontes da mesma verdade, uma delas envelhece.
    dono: MOVER_URL,
    ttl_s: TTL_MS / 1000,
