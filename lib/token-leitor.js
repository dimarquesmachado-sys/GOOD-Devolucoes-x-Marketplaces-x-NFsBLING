...
function vereditoDeCorte() {
  ...
  for (const eixo of ['good/bling', 'good/ml']) {
    ...
    const retryTotal = m.retry_apos_invalidar.ok + m.retry_apos_invalidar.falhou;
    if (invTotal > retryTotal) {
      ...
      const so403 = m.invalidacoes.por_403 > 0 && m.invalidacoes.por_401 === 0;
      motivos.push(`${invTotal - retryTotal} invalidacao(oes) sem retry conhecido `
        ...
        + (so403
          ...
          : ` — por_401=${m.invalidacoes.por_401}, por_403=${m.invalidacoes.por_403}`));
    }
