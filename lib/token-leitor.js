...
function vereditoDeCorte() {
  ...
  for (const eixo of ['good/bling', 'good/ml']) {
    ...
    if (pol === 'remoto') { saida[eixo] = { pronto: true, por_que: 'ja cortado' }; continue; }
    ...
    if (m.fonte.remoto + m.fonte.cache === 0) motivos.push('nunca leu do dono');
    ...
    saida[eixo] = motivos.length
      ...
      : { pronto: true, por_que: `${m.fonte.remoto + m.fonte.cache} leituras do dono, zero fallback local` };
  }
  return saida;
}
