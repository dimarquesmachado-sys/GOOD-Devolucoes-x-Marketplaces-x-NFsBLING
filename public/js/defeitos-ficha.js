...
(function () {
  ...
  // A GOOD serve tudo na raiz (a AMB e que fica sob /amb), entao aqui o
  // caminho das rotas nao leva prefixo nenhum.
  var BASE = '';
  ...
  window.voltarDefeitos = function () {
    ...
    else if (v.tipo === 'fila') abrirFilaPedidos(true, v.arg === true);   // v4.74 - volta pra ABA em que ele estava
  };
    ...
    try {
      ...
      el.innerHTML = aviso + itens.map(function (it, i) {
        ...
        return '<div onclick="abrirFichaDefeito(\'' + esc(it.id) + '\')" '
          ...
          // ⚠️ b301.1: a concatenacao estava DENTRO da string — o id saia
          // literal (`ficha-" + esc(x.id) + "`) e as aspas duplas quebravam
          // o HTML do card. A lista inteira caia em "erro ao buscar".
          //
          // Peguei porque o dono testou; `node --check` nao acusa (e string
          // valida) e nenhum teste montava o card de verdade.
  ...
  function linhaPedido(p, comAcoes, daAbaArquivadas) {
    ...
    var h = '<div style="border:2px solid ' + cor + ';border-radius:10px;overflow:hidden;margin-bottom:8px;">'
      + (p.defeito_id
          // v4.90 - o banner ja abria a ficha, mas parecia so um titulo. Agora
      ...
      // v4.78 (pedido do Diego) - o botao passa a se CHAMAR "Arquivar",
  ...
  // v4.73 (pedido do Diego) - a fila ganhou DUAS ABAS: o que ainda esta na
  ...
  window.abrirFilaPedidos = async function (voltando, verArquivadas) {
    ...
    // v4.74 (review do Codex) - a aba vai na pilha de navegacao: abrir
    // uma peca a partir de "Ja tratadas" e voltar caia sempre em "Na fila".
    registrar('fila', arq, voltando);
    // v4.74 - resposta ATRASADA nao pinta a tela: trocar de aba antes de
    ...
    if (meuToken !== window._filaToken) return;      // v4.74 - chegou tarde, ignora
    ...
    var html = topo('📥 Solicitações do Galpão')
      ...
      + '<div style="margin-bottom:11px;">'
        ...
        + aba('🗂️ Arquivadas', arq, true)   // v4.78 - era "Já tratadas"
      + '</div>'
  ...
  // v4.73 - tira da frente (ou devolve pra fila). SEM POPUP: o aviso de erro
  ...
  window.arquivarPedido = async function (id, arquivar, daAba) {
    ...
    try {
      ...
      if (r && r.ok) { abrirFilaPedidos(false, !!daAba); return; }   // v4.74
      if (aviso) {
